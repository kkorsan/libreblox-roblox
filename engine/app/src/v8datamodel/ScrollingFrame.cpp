...
namespace RBX
{
	...
	bool ScrollingFrame::doProcessGamepadEvent(const shared_ptr<InputObject>& event, const bool inputOver)
	{
		...
		if (event->getKeyCode() == KC_GAMEPAD_THUMBSTICK1 || event->isDPadEvent())
		{
			...
			if (event->isDPadEvent() && event->getUserInputState() == InputObject::INPUT_STATE_BEGIN)
			{
				...
				{
				case KC_GAMEPAD_DPADDOWN: scrollingDelta = Vector2(0,-1.0f); break;
				case KC_GAMEPAD_DPADUP:	scrollingDelta = Vector2(0,1.0f); break;
				case KC_GAMEPAD_DPADLEFT:	scrollingDelta = Vector2(-1.0f,0); break;
				case KC_GAMEPAD_DPADRIGHT:scrollingDelta = Vector2(1.0f,0); break;
				default:
					break;
				}
			}
			else if (event->getKeyCode() == KC_GAMEPAD_THUMBSTICK1)
			{
