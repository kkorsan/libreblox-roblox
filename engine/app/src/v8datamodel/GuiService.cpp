...
namespace RBX
{
	...
	bool GuiService::processKeyDown(const shared_ptr<RBX::InputObject>& event)
	{
		...
		if(event->isTextCharacterKey() || (UserInputService::IsUsingNewKeyboardEvents() && event->getUserInputType() == RBX::InputObject::TYPE_KEYBOARD))
		{
			...
			{
				if(event->modifiedKey == '/' || (UserInputService::IsUsingNewKeyboardEvents() && (event->getKeyCode() == KC_SLASH || event->getKeyCode() == KC_KP_DIVIDE)) )
				{
			...
			{
				case KC_INSERT:	if(dispatchKey(KEY_INSERT)) return true; break;
				case KC_HOME:		if(dispatchKey(KEY_HOME)) return true; break;
				case KC_END:		if(dispatchKey(KEY_END)) return true; break;
				case KC_PAGEUP:	if(dispatchKey(KEY_PAGEUP)) return true; break;
				case KC_PAGEDOWN:	if(dispatchKey(KEY_PAGEDOWN)) return true; break;
                default:
                    break;
			}
		return false;
