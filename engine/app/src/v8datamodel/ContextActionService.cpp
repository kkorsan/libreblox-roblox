...
static Reflection::BoundFuncDesc<ContextActionService, void(InputObject::UserInputType, KeyCode)> func_bindActivate(&ContextActionService::bindActivate, "BindActivate", "userInputTypeForActivation", "keyCodeForActivation", KC_UNKNOWN, Security::None);
static Reflection::BoundFuncDesc<ContextActionService, void(InputObject::UserInputType, KeyCode)> func_unbindActivate(&ContextActionService::unbindActivate, "UnbindActivate", "userInputTypeForActivation", "keyCodeForActivation", KC_UNKNOWN, Security::None);
    ...
    if (inputBindings && inputBindings->values.size() > 0 )
    {
        ...
        for (Reflection::ValueArray::iterator tupleIter = inputBindingsValues.begin(); tupleIter != inputBindingsValues.end(); ++tupleIter)
        {
            ...
            if (inputObjectIsKeyEvent)
            {
                ...
                if (keyCode != KC_UNKNOWN && inputObject->getKeyCode() == keyCode)
                {
                    return true;
                }
				...
				else if (iter->first.compare(activateGuid) == 0)
				{
					...
					if (canActivate && !menuIsOpen)
					{
						if (UserInputService* inputService = RBX::ServiceProvider::find<UserInputService>(this))
						{
							...
							if (inputObject->getKeyCode() == KC_GAMEPAD_BUTTONR2 || inputObject->getKeyCode() == KC_GAMEPAD_BUTTONL2)
							{
