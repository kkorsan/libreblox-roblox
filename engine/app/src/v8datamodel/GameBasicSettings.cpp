...
static Reflection::PropDescriptor<GameBasicSettings, float> prop_cameraSensitivity("CameraSensitivity",category_Data, &GameBasicSettings::getCameraSensitivity, &GameBasicSettings::setCameraSensitivity);
...
float GameBasicSettings::getCameraSensitivity() const
{
	return cameraSensitivity;
}

void GameBasicSettings::setCameraSensitivity(float value)
{
	try {
	...
	{
		RBX::StandardOut::singleton()->printf(RBX::MESSAGE_ERROR, "Insufficient permissions to set CameraSensitivity");
		throw e;
	}

	value = G3D::clamp(value,0.2f, 10.0f);
	if (value != cameraSensitivity)
	{
		cameraSensitivity = value;
		raisePropertyChanged(prop_cameraSensitivity);
	}
}
