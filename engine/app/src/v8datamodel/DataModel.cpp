...
uint64_t DataModel::LegacyLock::mainThreadId = ~0ULL;
...
private:
	...
	void step(Task& task)
	{
		...
		{
			RBX::StandardOut::singleton()->printf(RBX::MESSAGE_ERROR, "Exception in DataModel task: %s", e.what());
		}
		catch (...)
		{
			RBX::StandardOut::singleton()->printf(RBX::MESSAGE_ERROR, "Unknown exception in DataModel task");
		}
	}
...
bool DataModel::canRenderMouse()
{
	...
	{
		if (UserInputBase* hardwareDevice = service->getHardwareDevice())
		{
			if (hardwareDevice->isHardwareCursorActive())
			{
				return false;
			}
		}
...
ContentId DataModel::getRenderMouseCursor()
{
	...
	else
	{
		...
		{
			return userInputService->getDefaultMouseCursor(false);
		}
	...
	case InputObject::TYPE_KEYBOARD:
		{
...

			{
				...
				else
				{
					...
					case KC_F5: // TODO: use for run command in studio
						if ( !RBX::GameBasicSettings::singleton().inStudioMode() && event->isKeyPressedWithShiftEvent(event->getKeyCode()) )
					...
					case KC_SYSREQ: // This is technically print ...... directx doesn't have a print key code :(
					case KC_PRINT:
...
GuiResponse DataModel::processProfilerEvent(const shared_ptr<InputObject>& event)
{
	...
	if (event->isKeyDownEvent())
	{
        unsigned int ctrlOrCmd = KC_KMOD_LCTRL | KC_KMOD_RCTRL | KC_KMOD_LMETA | KC_KMOD_RMETA;

		if (event->isKeyDownEvent(KC_F6) && (event->mod & ctrlOrCmd))
			return Profiler::toggleVisible() ? GuiResponse::sunk() : GuiResponse::notSunk();

		if (event->isKeyDownEvent(KC_P) && (event->mod & ctrlOrCmd))
...
std::string DataModel::getMetric(const std::string& valueName) const
{
	...
	else if		(valueName == "Video Memory")			    return Log::formatMem(tempMetric->getMetricValue(valueName));
...
bool DataModel::currentThreadHasWriteLock() const
{
	...
	return writeRequestingThread == std::this_thread::get_id();
}
	...
	: dataModel(DataModel::get(context))
{
	if (dataModel)
	{
		...
		RBXASSERT(dataModel->writeRequestingThread == std::thread::id());

		dataModel->write_requested = 1;
		dataModel->writeRequestingThread = std::this_thread::get_id();
	}
}
...
DataModel::scoped_write_request::~scoped_write_request()
{
	if (dataModel)
	{
		...
		RBXASSERT(dataModel->read_requested == 0);
		RBXASSERT(dataModel->writeRequestingThread == std::this_thread::get_id());

		dataModel->write_requested = 0;
		dataModel->writeRequestingThread = std::thread::id();
	}
}
...
DataModel::scoped_write_transfer::scoped_write_transfer(Instance* context)
	: dataModel(DataModel::get(context))
	, oldWritingThread(std::thread::id())
{
	if (dataModel)
	{
		RBX::mutex::scoped_lock lock(dataModel->debugLock);
