...
namespace RBX
{
    ...
    void GamepadService::createControllerKeyMapForController(int controllerIndex)
    {
        ...
        controllerKeyMap[RBX::KC_GAMEPAD_BUTTONA] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_BUTTONA, gamepadType);
        controllerKeyMap[RBX::KC_GAMEPAD_BUTTONB] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_BUTTONB, gamepadType);
        controllerKeyMap[RBX::KC_GAMEPAD_BUTTONX] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_BUTTONX, gamepadType);
        controllerKeyMap[RBX::KC_GAMEPAD_BUTTONY] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_BUTTONY, gamepadType);
        controllerKeyMap[RBX::KC_GAMEPAD_BUTTONSTART] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_BUTTONSTART, gamepadType);
        controllerKeyMap[RBX::KC_GAMEPAD_BUTTONSELECT] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_BUTTONSELECT, gamepadType);

        controllerKeyMap[RBX::KC_GAMEPAD_BUTTONL1] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_BUTTONL1, gamepadType);
        controllerKeyMap[RBX::KC_GAMEPAD_BUTTONR1] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_BUTTONR1, gamepadType);

        controllerKeyMap[RBX::KC_GAMEPAD_BUTTONL2] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_BUTTONL2, gamepadType);
        controllerKeyMap[RBX::KC_GAMEPAD_BUTTONR2] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_BUTTONR2, gamepadType);

        controllerKeyMap[RBX::KC_GAMEPAD_BUTTONL3] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_BUTTONL3, gamepadType);
        controllerKeyMap[RBX::KC_GAMEPAD_BUTTONR3] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_BUTTONR3, gamepadType);

        controllerKeyMap[RBX::KC_GAMEPAD_DPADUP] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_DPADUP, gamepadType);
        controllerKeyMap[RBX::KC_GAMEPAD_DPADDOWN] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_DPADDOWN, gamepadType);
        controllerKeyMap[RBX::KC_GAMEPAD_DPADLEFT] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_DPADLEFT, gamepadType);
        controllerKeyMap[RBX::KC_GAMEPAD_DPADRIGHT] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_DPADRIGHT, gamepadType);

        controllerKeyMap[RBX::KC_GAMEPAD_THUMBSTICK1] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_THUMBSTICK1, gamepadType);
        controllerKeyMap[RBX::KC_GAMEPAD_THUMBSTICK2] = createInputObjectForGamepadKeyCode(RBX::KC_GAMEPAD_THUMBSTICK2, gamepadType);
	...
	void GamepadService::onInputChanged(const shared_ptr<Instance>& event)
	{
		if (InputObject* inputObject = Instance::fastDynamicCast<InputObject>(event.get()))
		{
			...
			if (event->getKeyCode() == RBX::KC_GAMEPAD_THUMBSTICK1)
			{
	...
	Vector2 GamepadService::getGuiSelectionDirection(const shared_ptr<InputObject>& event)
	{
		...
		else if(event->isDPadEvent())
		{
			if (event->getUserInputState() == InputObject::INPUT_STATE_BEGIN)
			{
				...
				{
				case KC_GAMEPAD_DPADUP:
					inputVector = Vector2(0,-1);
					break;
				case KC_GAMEPAD_DPADDOWN:
					inputVector = Vector2(0,1);
					break;
				case KC_GAMEPAD_DPADLEFT:
					inputVector = Vector2(-1,0);
					break;
				case KC_GAMEPAD_DPADRIGHT:
					inputVector = Vector2(1,0);
					break;
				default:
					break;
	...
	GuiResponse GamepadService::trySelectGuiObject(const Vector2& inputVector, const shared_ptr<InputObject>& event, BasePlayerGui* guiToProcess)
	{
		...
		if (inputVector == Vector2::zero())
		{
            if ( event && (isNavigationGamepad(event->getUserInputType()) || isCoreGuiProcessing) &&
                (event->getKeyCode() == RBX::KC_GAMEPAD_THUMBSTICK1 || event->isDPadEvent()) )
            {
			...
			if ( event && (event->getKeyCode() == RBX::KC_GAMEPAD_THUMBSTICK1 || event->isDPadEvent()) )
			{
	...
	{
		if (autoGuiSelectionAllowed && event->getKeyCode() == RBX::KC_GAMEPAD_BUTTONSELECT && event->getUserInputState() == InputObject::INPUT_STATE_BEGIN)
		{
