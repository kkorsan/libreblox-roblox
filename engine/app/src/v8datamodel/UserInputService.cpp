...
namespace RBX {
		...
		recenterUserHeadCFrameRequested(false)
    {
		...
		#ifdef _WIN32
        modPairs.push_back(std::make_pair(KC_LCTRL, KC_KMOD_LCTRL));
		modPairs.push_back(std::make_pair(KC_RCTRL, KC_KMOD_RCTRL));
		#elif __APPLE__
		modPairs.push_back(std::make_pair(KC_LMETA, KC_KMOD_LMETA));
		modPairs.push_back(std::make_pair(KC_RMETA, KC_KMOD_RMETA));
		#endif
		modPairs.push_back(std::make_pair(KC_LSHIFT, KC_KMOD_LSHIFT));
		modPairs.push_back(std::make_pair(KC_RSHIFT, KC_KMOD_RSHIFT));
		modPairs.push_back(std::make_pair(KC_CAPSLOCK, KC_KMOD_CAPS));
	...
	{
	#if defined(RBX_PLATFORM_UWP) || defined(_WIN32) || (defined(__APPLE__) && !defined(RBX_PLATFORM_IOS)) || defined(__linux__)
	    // Use the newer keyboard/text-input semantics (SDL / platform text input) on desktop platforms
	    // so we can rely on platform/SDL-provided Unicode/IME handling instead of US-only modifier heuristics.
	    return true;
	#else
	    return false;
	#endif
    ...
    void UserInputService::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
    {
        ...
        if (newProvider)
        {
            ...
            rotationInputObject = RBX::Creatable<Instance>::create<InputObject>(InputObject::TYPE_GYRO, InputObject::INPUT_STATE_CHANGE, Vector3(0,0,0), KC_UNKNOWN, RBX::DataModel::get(newProvider));
            accelerationInputObject = RBX::Creatable<Instance>::create<InputObject>(InputObject::TYPE_ACCELEROMETER, InputObject::INPUT_STATE_CHANGE, Vector3(0,0,0), KC_UNKNOWN, RBX::DataModel::get(newProvider));
            gravityInputObject = RBX::Creatable<Instance>::create<InputObject>(InputObject::TYPE_ACCELEROMETER, InputObject::INPUT_STATE_CHANGE, Vector3(0,0,0), KC_UNKNOWN, RBX::DataModel::get(newProvider));
			...
			currentMousePosition = RBX::Creatable<Instance>::create<InputObject>(InputObject::TYPE_MOUSEMOVEMENT, InputObject::INPUT_STATE_CANCEL, Vector3(-10000,-10000,0), KC_UNKNOWN, RBX::DataModel::get(newProvider));
        }
    }
	...
	void UserInputService::updateLastInputType(const shared_ptr<InputObject>& inputObject)
	{
		...
		{
			if ( (inputObject->getKeyCode() == KC_GAMEPAD_THUMBSTICK1 || inputObject->getKeyCode() == KC_GAMEPAD_THUMBSTICK2 ||
				inputObject->getKeyCode() == KC_GAMEPAD_BUTTONR2 || inputObject->getKeyCode() == KC_GAMEPAD_BUTTONL2) &&
    ...
    void UserInputService::processTextboxInternal()
    {
        ...
        if (tmpTextboxEventsVector.size() > 0)
        {
            ...
            {
				shared_ptr<InputObject> dummyReturnInput = RBX::Creatable<Instance>::create<InputObject>(InputObject::TYPE_KEYBOARD, InputObject::INPUT_STATE_BEGIN, Vector3::zero(), KC_RETURN, RBX::DataModel::get(this));
    ...
    void UserInputService::setClipboardCallbacks(GetClipboardTextCallback getCb, SetClipboardTextCallback setCb)
    {
    ...
    {
        if (getClipboardTextCallback)
        {
            return getClipboardTextCallback();
        }
    ...
    void UserInputService::setClipboardText(const std::string& text)
    {
        if (setClipboardTextCallback)
        {
            setClipboardTextCallback(text);
            return;
        }
        ...
        if(::OpenClipboard(NULL))
        {
            ...
            if (hData)
            {
                char* dest = static_cast<char*>(::GlobalLock(hData));
...
#ifdef _WIN32
        modCodes.push_back(KC_KMOD_LCTRL);
        modCodes.push_back(KC_KMOD_RCTRL);
#elif __APPLE__
        modCodes.push_back(KC_KMOD_LMETA);
        modCodes.push_back(KC_KMOD_RMETA);
#else
        modCodes.push_back(KC_KMOD_LCTRL);
        modCodes.push_back(KC_KMOD_RCTRL);
#endif
	...
	char UserInputService::getModifiedKey(RBX::KeyCode key, RBX::ModCode mod)
	{
		...
		bool shift = ((mod & RBX::KC_KMOD_RSHIFT) != 0) || ((mod & RBX::KC_KMOD_LSHIFT) != 0);
		bool caps = ((mod & RBX::KC_KMOD_CAPS) != 0);

		//XOR
		bool isUpper = (shift && !caps) || (!shift && caps);

		if ((key >= RBX::KC_A) && (key <= RBX::KC_Z))
		{
			if (isUpper)
			{
				return 'A' + (key - RBX::KC_A);
			}
			else
			{
				return 'a' + (key - RBX::KC_A);
		...
		if ((key >= RBX::KC_SPACE) && (key <= RBX::KC_AT)) {
			if(shift)
			{
				switch(key)
				{
				case RBX::KC_1:
					return RBX::KC_EXCLAIM;
					break;
				case RBX::KC_2:
					return RBX::KC_AT;
					break;
				case RBX::KC_3:
					...
					break;
				case RBX::KC_4:
					return RBX::KC_DOLLAR;
					break;
				case RBX::KC_5:
					return RBX::KC_PERCENT;
					break;
				case RBX::KC_6:
					return RBX::KC_CARET;
					break;
				case RBX::KC_7:
					return RBX::KC_AMPERSAND;
					break;
				case RBX::KC_8:
					return RBX::KC_ASTERISK;
					break;
				case RBX::KC_9:
					return RBX::KC_LEFTPAREN;
					break;
				case RBX::KC_0:
					return RBX::KC_RIGHTPAREN;
					break;
                default:
                    break;
				}
		...
		if(shift)
		{
			...
			case RBX::KC_COMMA:
				...
				break;
			case RBX::KC_PERIOD:
				return RBX::KC_GREATER;
				break;
			case RBX::KC_SLASH:
				return RBX::KC_QUESTION;
				break;
			case RBX::KC_MINUS:
				return RBX::KC_UNDERSCORE;
				break;
			case RBX::KC_BACKQUOTE:
				return RBX::KC_TILDE;
				break;
			case RBX::KC_LEFTBRACKET:
				return RBX::KC_LEFTCURLY;
				break;
			case RBX::KC_RIGHTBRACKET:
				return RBX::KC_RIGHTCURLY;
				break;
			case RBX::KC_BACKSLASH:
				return RBX::KC_PIPE;
				break;
			case RBX::KC_EQUALS:
				return RBX::KC_PLUS;
