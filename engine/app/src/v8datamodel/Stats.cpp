...
namespace RBX
{
	namespace Stats
	{
		...
		void reportGameStatus(const std::string& status, bool blocking)
		{
			...
			httpPost(RBX::format("%sapi/client-status/set?browserTrackerId=%s&status=%s", baseUrl.c_str(), Http::urlEncode(browserTrackerId).c_str(), Http::urlEncode(status).c_str()), "", blocking, "*/*");
		}
...
static const bool jobsAsArray = true;
		...
		std::string StatsService::getDefaultReportUrl(const std::string& baseUrlInput, const std::string& shard) {
			// Use provided base url if present, otherwise try global base url, otherwise fall back to canonical logging service
			std::string baseUrl = baseUrlInput;
			if (baseUrl.empty())
				baseUrl = ::GetBaseURL();

			if (baseUrl.empty()) {
				// No base url configured; use canonical logging service as last resort
				return std::string("http://logging.service.roblox.com/Gatherer/LogEntry?Shard=") + shard;
			}

			// Trim whitespace and normalize comparisons
			boost::algorithm::trim(baseUrl);
			std::string lowerBase = baseUrl;
			boost::algorithm::to_lower(lowerBase);
			...
			if (lowerBase.compare(0, http.size(), http) == 0) {
				schemePrefix = http;
				insertPos = http.size();
			} else if (lowerBase.compare(0, https.size(), https) == 0) {
				schemePrefix = https;
				insertPos = https.size();
			} else {
			...
			// If this is the public www.roblox.com host, prefer the canonical logging host with the same scheme.
			if (lowerBase.find("www.roblox.com") != std::string::npos) {
				std::string loggingBase = schemePrefix + std::string("logging.service.roblox.com/");
				return loggingBase + "Gatherer/LogEntry?Shard=" + shard;
			}

			// If the base URL already references a logging host, just use it.
			if (lowerBase.find("logging.service.") != std::string::npos) {
				if (baseUrl.back() != '/')
					baseUrl += '/';
				return baseUrl + "Gatherer/LogEntry?Shard=" + shard;
			}

			// Insert logging.service as the top subdomain (after scheme)
			if (insertPos != std::string::npos && insertPos <= baseUrl.size()) {
				baseUrl.insert(insertPos, "logging.service.");
			} else {
				baseUrl = schemePrefix + std::string("logging.service.roblox.com/");
			}
		...
		{
			if (!customReportUrl.empty())
				return customReportUrl;

			// Prefer the ContentProvider's base URL if available and non-empty.
			if (ContentProvider* contentProvider = ServiceProvider::find<ContentProvider>(this))
			{
				const std::string cpBase = contentProvider->getBaseUrl();
				if (!cpBase.empty())
					return getDefaultReportUrl(cpBase, "Client");
				else
					RBX::StandardOut::singleton()->printf(RBX::MESSAGE_WARNING, "StatsService: ContentProvider base URL is empty; falling back to global BaseURL");
			}

			// Fallback to the global base URL set via SetBaseURL/GetBaseURL
			const std::string globalBase = ::GetBaseURL();
			if (!globalBase.empty())
				return getDefaultReportUrl(globalBase, "Client");

			// As a last resort, log and return the canonical logging service URL.
			RBX::StandardOut::singleton()->printf(RBX::MESSAGE_WARNING, "StatsService: No base URL configured; using fallback logging.service.roblox.com");
			return std::string("http://logging.service.roblox.com/Gatherer/LogEntry?Shard=Client");
		}
...
#include <stdint.h>
		...
		// Provide an explicit template specialization for unsigned long long so
		// code that instantiates Item::formatValue<unsigned long long> will
		// have a defined symbol (some translation units use unsigned long long
		// directly).
		#ifndef _WIN32
		template<>
		void Item::formatValue(const unsigned long long& value) {
			formatValue(double(value), "%llu", value);
		}
		#endif
