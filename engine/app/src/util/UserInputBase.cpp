...
void UserInputBase::getNavKeys(NavKeys& navKeys, const bool shouldSuppressNavKeys) const
{
	...
	{
		navKeys.forward_arrow	= keyDown(KC_UP);
		navKeys.backward_arrow	= keyDown(KC_DOWN);
		navKeys.left_arrow		= keyDown(KC_LEFT);
		navKeys.right_arrow		= keyDown(KC_RIGHT);

		navKeys.strafe_right_e	= keyDown(KC_E);
		navKeys.strafe_left_q	= keyDown(KC_Q);
		navKeys.strafe_right_e	= keyDown(KC_E);
		navKeys.strafe_left_q	= keyDown(KC_Q);
		navKeys.forward_asdw	= keyDown(KC_W);
		navKeys.backward_asdw	= keyDown(KC_S);
		navKeys.left_asdw		= keyDown(KC_A);
		navKeys.right_asdw		= keyDown(KC_D);
		navKeys.space			= keyDown(KC_SPACE);
		navKeys.backspace		= keyDown(KC_BACKSPACE);
		navKeys.shift			= keyDown(KC_LSHIFT) || keyDown(KC_RSHIFT);
	}
}
