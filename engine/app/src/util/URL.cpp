...
Url Url::fromString(const char* str)
{
    ...
    if (!host_end)
    {
        ...
        // split host and port if present (e.g. example.com:8080 or [::1]:8080)
        {
            std::string hostport = url.host_;
            url.port_.clear();
            if (!hostport.empty())
            {
                // IPv6 literal with brackets: [::1]:8080
                if (hostport.front() == '[')
                {
                    ...
                    if (closeBracket != std::string::npos)
                    {
                        if (closeBracket + 1 < hostport.size() && hostport[closeBracket + 1] == ':')
                            url.port_ = hostport.substr(closeBracket + 2);
                        // keep bracketed literal in host_ (normalization will lowercase if appropriate)
                        url.host_ = hostport.substr(0, closeBracket + 1);
                    }
                }
                else
                {
                    // For typical host:port, take last ':' as separator, but only if there is a single ':' (i.e. not an unbracketed IPv6)
                    auto pos = hostport.rfind(':');
                    if (pos != std::string::npos && hostport.find(':') == pos)
                    {
                        url.port_ = hostport.substr(pos + 1);
                        url.host_ = hostport.substr(0, pos);
                    }
                }
            }
        }
...
{
    return query_.empty() || RFC3986::checkStringIsOfClass(query_, RFC3986::CharClassBit_QueryOrFragment);
}

bool Url::hasValidFragment() const
{
    return fragment_.empty() || RFC3986::checkStringIsOfClass(fragment_, RFC3986::CharClassBit_QueryOrFragment);
}
...
bool Url::isLocalhost() const
{
    // Normalize host for comparison: strip surrounding brackets for IPv6 literals
    std::string h = host_;
