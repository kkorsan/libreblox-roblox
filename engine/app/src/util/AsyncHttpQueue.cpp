...
// winhttp.h removed — status code defines moved to a generic header shared across platforms
#endif
...
static void callback(AsyncHttpQueue::CallbackWrapper f, Instance* owner, AsyncHttpQueue::RequestResult result, shared_ptr<std::string> response, shared_ptr<std::exception> exception)
{
	...
	catch (...)
	{
		try
		{
			RBX::StandardOut::singleton()->printf(RBX::MESSAGE_ERROR, "Unknown exception in ContentProvider callback");
		}
		catch (...)
		{
			// Ignore logging exceptions
...
void AsyncHttpQueue::processRequests(boost::weak_ptr<AsyncHttpQueue> weakHttpQueue, RequestHandle request, boost::shared_ptr<rbx::spin_mutex> criticalSection)
{
	...
	catch (const std::exception& e)
	{
		try
		{
			RBX::StandardOut::singleton()->printf(RBX::MESSAGE_ERROR, "Exception in AsyncHttpQueue::processRequests: %s", e.what());
		}
		catch (...)
		{
			// Ignore logging exceptions
	...
	catch (...)
	{
		try
		{
			RBX::StandardOut::singleton()->printf(RBX::MESSAGE_ERROR, "Unknown exception in AsyncHttpQueue::processRequests");
		}
		catch (...)
		{
			// Ignore logging exceptions
