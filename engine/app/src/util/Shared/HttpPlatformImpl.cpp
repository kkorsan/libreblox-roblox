...
bool logCurlError(void* caller, const char* curlOperation, Code curlCode, bool doThrow)
{
    ...
    if (!curlCodeOkay(curlCode))
    {
        if (doThrow)
        {
            throw RBX::runtime_error("CURL error (%s, %p): %s", curlOperation, caller, getCurlStrerror(curlCode));
        }
        else
        {
...
public:
    ...
    long perform(std::string& response, bool zeroLatencyCaching)
    {
        ...
        {
            for (int redirects = 0;
                (302 == responseCode || 301 == responseCode || 303 == responseCode || 307 == responseCode || 308 == responseCode) && (redirects < DFInt::HttpMaxRedirects || -1 == DFInt::HttpMaxRedirects);
                ++redirects)
            {
                cacheFailed = false;
                responseHeaders.str("");
