...
void SoundService::getCpuStats(CpuStats& stats) const
{
	if (!system)
	{
		...
		stats.geometry = 0;
		stats.update = 0;
		stats.total = 0; // Initialize total as well
		return;
	}

	// FMOD's getCPUUsage expects a single FMOD_CPU_USAGE struct pointer.
	// The previous call signature is deprecated or incorrect for the current FMOD version.
	FMOD_CPU_USAGE fmodStats;
	system->getCPUUsage(&fmodStats);

	// The values returned by FMOD_CPU_USAGE members are often scaled (e.g., as percentages * 100).
	// Apply the original scaling factor (0.01) to convert them to the expected range for CpuStats.
	stats.dsp = 0.01f * fmodStats.dsp;
	stats.geometry = 0.01f * fmodStats.geometry;
	stats.stream = 0.01f * fmodStats.stream;
	stats.update = 0.01f * fmodStats.update;
	// FMOD_CPU_USAGE struct typically does not have a 'total' member.
	// Calculate total from the other CPU usage components.
	stats.total = stats.dsp + stats.geometry + stats.stream + stats.update;
}
