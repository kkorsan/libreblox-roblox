...
namespace RBX
{
	...
	private:
		...
		// Selection support
		// When selectionActive is true, [selectionStart, selectionEnd) describes the selected substring.
		// selectionStart is inclusive and selectionEnd is exclusive.
		bool selectionActive;
		int selectionStart;
		int selectionEnd;
		bool isMouseSelecting; // true while the user is dragging to select text
		bool selectionRepeatActive;
		...
		bool isCopyCommand(const shared_ptr<InputObject>& event);
		bool isCutCommand(const shared_ptr<InputObject>& event);
