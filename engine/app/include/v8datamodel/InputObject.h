...
namespace RBX {
    ...
    public:
		...
		bool isKeyPressedWithShiftEvent() const
		{
			return (inputType == TYPE_KEYBOARD
				&& (inputState != INPUT_STATE_END && inputState != INPUT_STATE_NONE)
				&& ((mod & KC_KMOD_LSHIFT) || (mod & KC_KMOD_RSHIFT)));
		}
		...
		bool isKeyPressedWithCtrlEvent() const
		{
			return (inputType == TYPE_KEYBOARD
				&& (inputState != INPUT_STATE_END && inputState != INPUT_STATE_NONE)
				&& ((mod & KC_KMOD_LCTRL) || (mod & KC_KMOD_RCTRL)));
		}
		...
		bool isDPadEvent() const
		{
			return isGamepadEvent() &&
				(keyCode == KC_GAMEPAD_DPADDOWN || keyCode == KC_GAMEPAD_DPADUP || keyCode == KC_GAMEPAD_DPADLEFT || keyCode == KC_GAMEPAD_DPADRIGHT);
		}
