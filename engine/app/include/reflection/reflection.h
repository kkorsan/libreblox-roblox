...
#define RBX_REGISTER_CLASS(Class)		template<> RBX::Reflection::ClassRegistrar<Class> RBX::Reflection::ClassRegistrar<Class>::registrar(0)
		...
		public:
			...
			template<typename Get, typename Set>
			static typename boost::enable_if_c<!boost::is_integral<Get>::value && !boost::is_integral<Set>::value, std::unique_ptr< typename Reflection::TypedPropertyDescriptor<V>::GetSet > >::type
			getset(Get get, Set set)
			{
				return std::unique_ptr< typename Reflection::TypedPropertyDescriptor<V>::GetSet >(new GetSetImpl<Get, Set>(get, set));
			}

            // Overload for read-only case: second parameter is an integral NULL marker
            template<typename Get, typename Set>
            static typename boost::enable_if< boost::is_integral<Set>, std::unique_ptr< typename Reflection::TypedPropertyDescriptor<V>::GetSet > >::type
            getset(Get get, Set /*set*/)
            {
                return std::unique_ptr< typename Reflection::TypedPropertyDescriptor<V>::GetSet >(new GetImpl<Get>(get));
            }
            // Overload for write-only case: first parameter is an integral NULL marker
            template<typename Get, typename Set>
            static typename boost::enable_if< boost::is_integral<Get>, std::unique_ptr< typename Reflection::TypedPropertyDescriptor<V>::GetSet > >::type
            getset(Get /*get*/, Set set)
            {
