 ...
 * - host() is expected to be a domain name (or an IP address / literal).
 * - port() is supported and returns the port number string (without leading ':') when present.
 ...
 * - Userinfo (user:password@) is still not supported.
 ...
 *   - any component contains invalid characters (see RFC3986) or malformed %-sequences
 */
...
public:
    ...
    // port number without leading ':'; empty if not present
    ...
    // Returns true when the host refers to localhost (\"localhost\" or 127.0.0.1 or ::1)
