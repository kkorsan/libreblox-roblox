...
RBX::Rect2D AdornRender::getViewport() const
{
	...
	return Rect2D::xywh(0, 0, visualEngine->getViewWidth() / viewScale, visualEngine->getViewHeight() / viewScale);
}
...
void AdornRender::rect2dImpl(const Vector2& x0y0, const Vector2& x1y0, const Vector2& x0y1, const Vector2& x1y1, const Vector2& tex0, const Vector2& tex1, const Color4& color)
{
    ...
    Vector2 px0y0(x0y0 * viewScale);
    Vector2 px1y0(x1y0 * viewScale);
    Vector2 px0y1(x0y1 * viewScale);
    Vector2 px1y1(x1y1 * viewScale);
...
{
	float viewScale = visualEngine->getDevice()->getCaps().viewScale;
	visualEngine->getVertexStreamer()->line(p0.x * viewScale, currentHeight - (p0.y * viewScale), p1.x * viewScale, currentHeight - (p1.y * viewScale), &color[0]);
}
...
void AdornRender::convexPolygon2d(const Vector2* v, int countv, const Color4& color)
{
    ...
    {
        vertices[i] = Vector2(v[i].x * viewScale, currentHeight - (v[i].y * viewScale));
    }
