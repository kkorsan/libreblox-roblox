...
private:
    ...
    GeometryBatch createMesh(Device* device, FileMeshData* data)
    {
        ...
        if (Vertex* vbptr = static_cast<Vertex*>(vbuf->lock()))
        {
        ...
        if (unsigned short* ibptr = static_cast<unsigned short*>(ibuf->lock()))
        {
...
public:
	...
	void render(DeviceContext* context, const shared_ptr<Framebuffer>& framebuffer)
    {
        ...
        for (size_t i = 0; i < layers.size(); ++i)
        {
			...
			if (program && *ld.mesh && (*ld.mesh)->getGeometry() && ld.texture.getTexture())
			{
