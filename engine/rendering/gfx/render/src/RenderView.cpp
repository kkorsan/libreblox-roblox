...
static const unsigned validGraphicsModes = 4;
...
static Vector2 computeCanvasSize(Device* device)
{
	...
	{
		float viewScale = device->getCaps().viewScale;

		return Vector2(framebuffer->getWidth(), framebuffer->getHeight());
	}
	else
	{
		return Vector2();
    ...
    , frameDataCallback(FrameDataCallback())
{
    ...
    {
        if (gGraphicsModesTranslation[i].settingsItem == graphicsMode || (!translatioFound && gGraphicsModesTranslation[i].settingsItem == CRenderSettings::OpenGL))
        {
            try
            {
                device.reset(Device::create(gGraphicsModesTranslation[i].api, context->window, context->display));
                translatioFound = true;
                break;
            }
            catch (const std::exception& e)
            {
                RBX::StandardOut::singleton()->printf(RBX::MESSAGE_WARNING, "Graphics mode %d failed: %s", gGraphicsModesTranslation[i].settingsItem, e.what());
                // Try next graphics mode
            }
        }
    }
...
void RenderView::setVSync(bool enabled)
{
    visualEngine->getDevice()->setVSync(enabled);
}
...
void RenderView::bindWorkspace(boost::shared_ptr<RBX::DataModel> dataModel)
{
	...
	if (this->dataModel)
	{
        ...
        {
            float viewScale = visualEngine->getDevice()->getCaps().viewScale;
            this->dataModel->setInitialScreenSize(computeCanvasSize(visualEngine->getDevice()) / viewScale);
        }
...
{
    if (isShuttingDown || !dataModel) return;
    ...
    if (updateViewport)
	{
		...
		if (Camera* camera = dataModel->getWorkspace()->getCamera())
		{
            ...
            float viewScale = visualEngine->getDevice()->getCaps().viewScale;
			camera->setViewport(newViewport / viewScale);
		}
	}
...
void RenderView::renderPerformImpl(double timeJobStart, Framebuffer* mainFramebuffer)
{
    ...
    if (DeviceContext* context = visualEngine->getDevice()->beginFrame())
    {
        ...
        // Video recording has been removed. Use external tools (e.g. OBS) to capture gameplay.
        // Previously the renderer forwarded frames to an in-app VideoRecorder; that logic is gone.
...
void RenderView::setShuttingDown(bool value)
{
...
float RenderView::getViewScale() const
{
    return device->getCaps().viewScale;
}
