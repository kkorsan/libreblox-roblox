    ...
    , meshContentProvider(0)
{
    ...
    for (Text::Font font = Text::FONT_LEGACY; font != Text::FONT_LAST; font=Text::Font(font+1))
    {
        ...
        typesetters[font].reset(new TypesetterDynamic(glyphAtlas.get(), textureManager.get(), ContentProvider::assetFolder() + kFontTTFPaths[font], legacyHeightScale, (unsigned)font, device->getCaps().viewScale));
    }
...
void VisualEngine::setViewport(int width, int height)
{
    ...
    for (int i = Text::FONT_LEGACY; i != Text::FONT_LAST; ++i)
    {
        if (TypesetterDynamic* td = dynamic_cast<TypesetterDynamic*>(typesetters[i].get()))
        {
            td->setRetinaScale(viewScale);
        }
    }
}
