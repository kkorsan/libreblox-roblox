...
, postProcessSettings(PostProcessSettings())
{
	...
	// if (!FFlag::RenderVR)
	// {
	// 	if (device->getCaps().viewScale > 1.0f)
	// 	{
...
void SceneManager::updateMSAA(unsigned width, unsigned height)
{
	...
	try
	{
		...
		{
            bool msaaEnabled = frm->getAntialiasingMode() == CRenderSettings::AntialiasingOn;

            // Legacy logic fallback: if strictly high DPI and 4x supported, enable it (if auto/on wasn't explicitly checked above, but here we cover all 'On' cases)
            // Actually, let's stick to the settings.

            if (msaaEnabled && device->getCaps().maxSamples >= 2)
            {
                unsigned int samples = std::min(device->getCaps().maxSamples, 8u);
