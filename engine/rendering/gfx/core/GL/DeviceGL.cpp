...
static DeviceCapsGL createDeviceCaps(ContextGL* context, const std::set<std::string>& extensions)
{
    ...
    caps.viewScale = context->getMainFramebufferScale();
...
DeviceGL::DeviceGL(void* windowHandle, void* display)
    : frameTimeQueryId(0)
	...
	, vrEnabled(true)
{
	glContext.reset(ContextGL::create(windowHandle, display));
...
std::string DeviceGL::getFeatureLevel()
{
    ...
    std::string glString = glVersion ? glVersion : "";
...
bool DeviceGL::validate()
{
    ...
    caps.viewScale = glContext->getMainFramebufferScale();
...
void DeviceGL::endFrame()
{
	...
	FASTLOG(FLog::Graphics, "DeviceGL::endFrame - calling swapBuffers");
    glContext->swapBuffers();
    FASTLOG(FLog::Graphics, "DeviceGL::endFrame - swapBuffers completed");
}
...
void DeviceGL::setVSync(bool enabled)
{
    glContext->setVSync(enabled);
}
