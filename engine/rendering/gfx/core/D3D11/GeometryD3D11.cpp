...
namespace Graphics
{
    ...
    {
        ID3D11Device* device11 = static_cast<DeviceD3D11*>(this->device)->getDevice11();

        D3D11_BUFFER_DESC bd = {};
        bd.Usage = gBufferUsageD3D11[this->usage].usage; 
        bd.ByteWidth = this->elementSize * this->elementCount;
        bd.BindFlags = bindFlags;
        bd.CPUAccessFlags = gBufferUsageD3D11[this->usage].cpuAccess;
        bd.MiscFlags = 0;
    ...
    template <typename Base> void* GeometryBufferD3D11<Base>::lock(GeometryBuffer::LockMode mode)
    {
        ...
        if (this->usage == GeometryBuffer::Usage_Static)
        {
            locked = new char[this->elementSize * this->elementCount];
        }
        else
        {
            ID3D11DeviceContext* context11 = static_cast<DeviceD3D11*>(this->device)->getImmediateContext11();
            ...
            if (FAILED(hr))
            {
                FASTLOG2(FLog::Graphics, "Failed to lock VB (size %d): %x", this->elementCount * this->elementSize, hr);
                return NULL;
            }
    ...
    template <typename Base> void GeometryBufferD3D11<Base>::unlock()
    {
        ...
        if (this->usage == GeometryBuffer::Usage_Static)
        {
            upload(0, locked, this->elementCount * this->elementSize);
