...
bool RakString::Deserialize(BitStream *bs)
{
	...
	if (l>0)
	{
		if (l > 200000) // matches MAX_STRING_SIZE in Streaming.cpp
			return false;
...
{
	// Legacy version: assume a reasonably sized buffer for basic safety, but this is still risky.
	// 512 is a common stack buffer size in this codebase.
	return Deserialize(str, 512, bs);
}
bool RakString::Deserialize(char *str, unsigned int maxLen, BitStream *bs)
{
	bool b;
	unsigned short l;
	b=bs->Read(l);
	if (b && l>0)
	{
		if (l >= maxLen)
		{
			// Potential overflow! Read only what fits.
			b=bs->ReadAlignedBytes((unsigned char*) str, maxLen - 1);
			bs->IgnoreBytes(l - (maxLen - 1));
			str[maxLen - 1] = 0;
		}
		else
		{
...
{
	return DeserializeCompressed(str, 512, bs, readLanguageId);
}
bool RakString::DeserializeCompressed(char *str, unsigned int maxLen, BitStream *bs, bool readLanguageId)
{
	uint8_t languageId;
	if (readLanguageId)
		bs->ReadCompressed(languageId);
		...
		languageId=0;
	return StringCompressor::Instance()->DecodeString(str,maxLen,bs,languageId);
}
