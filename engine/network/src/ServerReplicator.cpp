...
void CheatHandlingServerReplicator::decodeHashItem(PmcHashContainer& netHashes, unsigned long long* securityTokens)
{
    ...
    if (!reportedHashItemTime && delta >= DFInt::ReportTimeLimit1)
    {
	    ...
	    hashItemAnalytics.addPoint("delta", static_cast<double>(delta));
	    hashItemAnalytics.addPoint("num", numHashItems);

	    hashItemAnalytics.addPoint("pingDelta", static_cast<double>(pingDelta));
	    hashItemAnalytics.addPoint("pingNum", numPingItems);

	    hashItemAnalytics.addPoint("mccDelta", static_cast<double>(mccDelta));
...
// --- Cross-platform helper functions for CPU intrinsics ---
// Place these helper functions at the top of the file or in an appropriate utility location
// within the RBX::Network namespace or an anonymous namespace inside the .cpp file.

// Cross-platform population count (popcnt)
#ifdef _WIN32
    #include <intrin.h> // For __popcnt
    inline unsigned int popcount_wrapper(unsigned int x) {
        return __popcnt(x);
    }
...
#else
    // Fallback implementation if no builtin is available
    inline unsigned int popcount_wrapper(unsigned int x) {
        // Count set bits using Brian Kernighan's algorithm or simple loop
        ...
        return count;
        /*
        // Alternative Brian Kernighan's algorithm (often faster):
        unsigned int count = 0;
        while (x) {
...
// Cross-platform CPUID
    ...
    inline void cpuid_wrapper(int cpuInfo[4], int function_id) {
        __cpuid(cpuInfo, function_id);
    }
// Add architecture check here: __get_cpuid only exists on x86/x64
...
#else
    // Fallback for ARM64, PowerPC, or other compilers
    inline void cpuid_wrapper(int cpuInfo[4], int function_id) {
        // Architecture-specific feature detection would go here.
        // For now, we return 0 to indicate features like SSE/POPCNT aren't present.
        cpuInfo[0] = cpuInfo[1] = cpuInfo[2] = cpuInfo[3] = 0;
    }
#endif
// --- End of cross-platform helpers ---
...
{
    // Use the cross-platform wrapper instead of __popcnt directly
    return (1 & popcount_wrapper(x & decodeKey));
}
...
void CheatHandlingServerReplicator::processRockyMccReport(const MccReport& report)
{
    ...
    if (!reportedMccItemTime && delta >= DFInt::ReportTimeLimit2)
    {
	    ...
	    mccItemAnalytics.addPoint("delta", static_cast<double>(delta));
    ...
    // Use the cross-platform wrapper instead of __cpuid directly
    int cpuid[4];
    cpuid_wrapper(cpuid, 1); // function_id 1

    // Check for POPCNT support (bit 23 of ECX)
    if (cpuid[2] & (1<<23))
    {
    ...
    {
        // If POPCNT is not supported, we cannot reliably decode the MCC report.
        // The original logic returned here, effectively ignoring the report
        // if the CPU feature isn't present. We maintain that behavior.
        return;
    }
...
void CheatHandlingServerReplicator::checkPingItemTime()
{
    ...
    if (!reportedPingItemTime && delta >= DFInt::ReportTimeLimit3)
    {
	    ...
	    pingItemAnalytics.addPoint("delta", static_cast<double>(delta));
...
void CheatHandlingServerReplicator::PlaceAuthenticationThreadImpl(int previousPlaceId, int requestedPlaceId)
{
	...
	sprintf_s(urlBuf, 2048, "%sapi/universes/validate-place-join?originPlaceId=%d&destinationPlaceId=%d", baseUrl.c_str(), previousPlaceId, requestedPlaceId);
