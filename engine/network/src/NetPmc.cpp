...
// Cross-platform bit rotation functions
#ifdef _WIN32
...
bool NetPmcServer::canSendChallenge(const Network::Replicator* rep) /*non-const*/
{
...
unsigned int NetPmcServer::generateDebugInfo(const Network::Replicator* rep, uint32_t& sent, uint32_t& recv, uint32_t& pending) const
{
...
{
    uint64_t correctDecrypted = Security::teaDecrypt(correct);
...
#ifdef _WIN32
    inline uint32_t byteswap_ulong(uint32_t val) {
...
#define LE(p) ( byteswap_ulong(*reinterpret_cast<const uint32_t*>(p)) )
    uint32_t in[16] = {LE(c),            LE(key),    LE(key+4),        LE(key+8),
        LE(key+12),       LE(c+4),    static_cast<uint32_t>(nonce&0xffffffff), static_cast<uint32_t>(nonce>>32),
        static_cast<uint32_t>(index&0xffffffff), static_cast<uint32_t>(index>>32),  LE(c+8),          LE(key+16),
