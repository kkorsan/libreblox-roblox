...
// Cross-platform inline function definition
#ifdef _WIN32
...
FORCE_INLINE std::vector<NetPmcChallenge> generateNetPmcKeys()
{
    ...
    Security::salsa20(reinterpret_cast<uint8_t*>(keys.data()), sizeof(NetPmcChallenge)*kNumChallenges, key, 2015);
    return keys;
}
...
public:
    ...
    bool canSendChallenge(const Network::Replicator* rep); /*non-const*/
    ...
    unsigned int generateDebugInfo(const Network::Replicator* rep, uint32_t& sent, uint32_t& recv, uint32_t& pending) const;
