...
namespace RBX
{
	namespace Network
	{
		...
		{
			static_assert( sizeof(T) == 0 ); // This is undefined.
		}
		...
		// Additional 64-bit specialisations to cover ‘long long’ forms on all ABIs
		template <>
		inline void readFastT(RakNet::BitStream& bitStream, long long& result) {
            static_assert(sizeof(long long) == sizeof(uint64_t));
            ReadFastBytes<0>::readFast<64, uint64_t>(bitStream, (uint64_t&)result);
		}

		template <>
		inline void readFastT(RakNet::BitStream& bitStream, unsigned long long& result) {
