...
public:
	...
	/// @param[in] maxLen Maximum number of characters to write to str, including the null terminator.
	static bool Deserialize(char *str, unsigned int maxLen, BitStream *bs);
	static bool Deserialize(char *str, BitStream *bs);

	/// Deserialize compressed string, written by SerializeCompressed
	/// \param[in] bs The bitstream to read from
	/// \param[in] readLanguageId encode the languageId variable in the stream. If false, 0 is assumed, and DeserializeCompressed will not look for this variable in the stream (saves bandwidth)
	bool DeserializeCompressed(BitStream *bs, bool readLanguageId=false);

	/// Static version of the DeserializeCompressed() function
	static bool DeserializeCompressed(char *str, unsigned int maxLen, BitStream *bs, bool readLanguageId=false);
