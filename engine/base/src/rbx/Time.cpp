...
static void checkDbg()
{
    ...
    return;
#else
    // IsDebuggerPresent() is the standard Win32 way to check for a debugger.
    // It is much more portable than manual PEB assembly.
...
return static_cast<double>(tinfo.numer/(double)tinfo.denom * 1e-9);
    ...
    // On Linux, CLOCK_MONOTONIC is the best choice for a high-resolution timer.
    // It is not affected by NTP adjustments and is suitable for measuring elapsed time.
    timespec res;
...
void startMMTimer()
{
	...
	MMRESULT result = timeSetEvent(
        timerRes,
        timerRes,
        // Cast the chosen function to LPTIMECALLBACK
        (LPTIMECALLBACK)(ticksPerCorrection >= 2 ? interpolatedCallback : directCallback),
        (DWORD_PTR)NULL,
        TIME_PERIODIC );
