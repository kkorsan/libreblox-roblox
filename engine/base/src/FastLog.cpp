...
namespace FLog
{
	...
	public:
		...
		virtual VariableKind getKind() = 0;
...
#pragma warning(pop)
	...
	template<class T> class ValueGetSet : public IValueGetSet
	{
        ...
        bool isSync;
	public:
		ValueGetSet(T* value, FastVarType _fastVarType, VariableKind _kind) : value(value), fastVarType(_fastVarType), kind(_kind), isSync(true) {}
...
#pragma warning(disable: 4996)
		...
		if(static_cast<int>(reinterpret_cast<intptr_t>(arg0)) == FLOAT_MARKER)
...
#pragma warning(pop)
	...
	template<class T> void RegisterVariable(const char* varName, T* value, bool** isSync, FastVarType fastVarType, VariableKind kind)
	{
		if(gVariables == NULL)
        ...
        ValueGetSet<T>* varValue = new ValueGetSet<T>(value, fastVarType, kind);
	...
	{
		RegisterVariable(groupName, groupVar, isSync, fastVarType, VARKIND_LOGGROUP);
		return 1;
	}

	int RegisterString(const char* groupName, std::string* groupVar, bool** isSync, FastVarType fastVarType)
	{
		RegisterVariable(groupName, groupVar, isSync, fastVarType, VARKIND_STRING);
		return 1;
	}

	int RegisterInt(const char* groupName, int* groupVar, bool** isSync, FastVarType fastVarType)
	{
		RegisterVariable(groupName, groupVar, isSync, fastVarType, VARKIND_INT);
		return 1;
	}

	int RegisterFlag(const char* flagName, bool* flagVar, bool** isSync, FastVarType fastVarType)
	{
		RegisterVariable(flagName, flagVar, isSync, fastVarType, VARKIND_FLAG);
		return 1;
	}
	...
	static void visitVariableEx(Variables::value_type pair, VariableVisitorEx visitor, void* context)
	{
		visitor(pair.first.c_str(), pair.second->get(), pair.second->getType(), pair.second->getKind(), context);
	}
	...
	void ForEachVariable(VariableVisitorEx visitor, void* context, FastVarType flagType)
	{
        if (flagType == FASTVARTYPE_ANY)
        {
		    std::for_each(gVariables->begin(), gVariables->end(), boost::bind(visitVariableEx, _1, visitor, context));
        }
        else
        {
            for (Variables::iterator iter = gVariables->begin(); iter != gVariables->end(); iter++)
            {
                if ((iter->second->getType() & flagType) > 0)
                {
                    visitor(iter->first.c_str(), iter->second->get(), iter->second->getType(), iter->second->getKind(), context);
                }
            }
        }
	}
