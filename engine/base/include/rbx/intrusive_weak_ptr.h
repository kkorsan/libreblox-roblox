...
namespace rbx
{
	...
	*/
	...
	public:
		...
		{
			if(p_ != 0) intrusive_ptr_add_weak_ref(p_);
		}
		...
		{
			if( p_ != 0 ) intrusive_ptr_weak_release(p_);
		}
		...
		{
			if (p_ && intrusive_ptr_try_lock(p_))
		...
		{
			return (!p_ || intrusive_ptr_expired(p_));
		}
