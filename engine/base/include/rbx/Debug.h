...
// This macro will log an assert failure instead of crashing. Usually you don't call it directly. Use RBXASSERT instead
...
//
	...
	#define RBXASSERT RBX_CRASH_ASSERT
        ...
        #include "TargetConditionals.h"
            ...
            #define RBXASSERTENABLED
        #endif
	#elif (defined(_DEBUG) && defined(_WIN32) && defined(_MSC_VER))		// MSVC Windows Debug
		#define RBXASSERT(expr) \
			((void) (!!(expr) || \
			((RBX::_internal::_debugHook != NULL) && (RBX::_internal::_debugHook(#expr, __FILE__, __LINE__))) || \
			(_ASSERTE(expr), 0)))
		#define RBXASSERTENABLED
	#elif (defined(_DEBUG) && defined(_WIN32) && defined(__GNUC__))		// GCC Windows Debug
		#define RBXASSERT(expr) \
			((void) (!!(expr) || \
			((RBX::_internal::_debugHook != NULL) && (RBX::_internal::_debugHook(#expr, __FILE__, __LINE__))) || \
			(__builtin_trap(), 0)))
