...
local function Initialize()
	...
	local function createGraphicsOptions()
        ...
        ------------------ Frame Rate Cap GUI Setup ------------------
        local FRAME_RATE_CAP_STRINGS = {"VSync", "30 FPS", "60 FPS", "120 FPS", "144 FPS", "165 FPS", "240 FPS", "Uncapped"}
        local currentCapValue = 1
        pcall(function() currentCapValue = settings().Rendering.FrameRateCap.Value end)
        
        -- Map Auto (0) to VSync (1)
        if currentCapValue == 0 then currentCapValue = 1 end
        
        local currentCapIndex = currentCapValue

        this.FrameRateCapFrame,
        this.FrameRateCapLabel,
        this.FrameRateCapSelector = utility:AddNewRow(this, "Frame Rate Cap", "Selector", FRAME_RATE_CAP_STRINGS, currentCapIndex)

        this.FrameRateCapSelector.IndexChanged:connect(function(newIndex)
            settings().Rendering.FrameRateCap = newIndex
        end)
	...
	local function createMouseOptions()
		local MouseSteps = 10
		local MinCameraSensitivity = 0.2

		-- equations below map a function to include points (0, 0.2) (5, 1) (10, 4)
		-- where x is the slider position, y is the mouse sensitivity
		local function translateEngineCameraSensitivityToGui(engineSensitivity)
			return math.floor((2.0/3.0) * (math.sqrt(75.0 * engineSensitivity - 11.0) - 2))
		end

		local function translateGuiCameraSensitivityToEngine(guiSensitivity)
			return 0.03 * math.pow(guiSensitivity,2) + (0.08 * guiSensitivity) + MinCameraSensitivity
		end

		local startMouseLevel = translateEngineCameraSensitivityToGui(GameSettings.CameraSensitivity)

		this.CameraSensitivityFrame,
		this.CameraSensitivityLabel,
		this.CameraSensitivitySlider = utility:AddNewRow(this, "Mouse Sensitivity", "Slider", MouseSteps, startMouseLevel)
		this.CameraSensitivitySlider:SetMinStep(1)

		this.CameraSensitivitySlider.ValueChanged:connect(function(newValue)
			GameSettings.CameraSensitivity = translateGuiCameraSensitivityToEngine(newValue)
		end)
	end
