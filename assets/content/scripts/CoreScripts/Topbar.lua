...
local function GetChatVisibleIconFlag()
    local chatVisibleIconSuccess, chatVisibleIconFlagValue = pcall(function()
        return settings():GetFFlag(
            "MobileToggleChatVisibleIcon")
    end)
    return chatVisibleIconSuccess and chatVisibleIconFlagValue == true
end

local defeatableTopbarSuccess, defeatableTopbarFlagValue = pcall(function()
    return settings():GetFFlag(
        "EnableSetCoreTopbarEnabled")
end)
    ...
    local function AnimateHurtOverlay()
        if hurtOverlay then
            ...
            if hurtOverlay:IsDescendantOf(game) then
                ...
                hurtOverlay:TweenSizeAndPosition(newSize, newPos, Enum.EasingDirection.Out, Enum.EasingStyle.Linear, 0,
                    true, function()
                        -- show the gui
                        ...
                        -- now tween the hide
                        if hurtOverlay:IsDescendantOf(game) then
                            hurtOverlay:TweenSizeAndPosition(newSize, newPos, Enum.EasingDirection.Out, Enum.EasingStyle
                                .Quad, 10, false, function()
