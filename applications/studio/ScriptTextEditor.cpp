...
void ScriptTextEditor::autoCompleteBlock(const QString& text, QString& indent)
{
	...
	QRegularExpression commentMatchingExp("(--)?\\[=*\\[");
    if (text.contains(QRegularExpression("\\brepeat\\b")) && !text.contains(QRegularExpression("\\buntil\\b")))
        foldEnd = QString("until");
	else if (commentMatchingExp.match(text).hasMatch() && !text.contains(commentMatchingExp.match(text).captured().replace('[', ']')))
		foldEnd = QString(commentMatchingExp.match(text).captured().replace('[', ']'));
...
void ScriptTextEditor::setAnalysisResult(const RBX::ScriptAnalyzer::Result& analysisResults)
{
	...
	for (size_t i = 0; i < analysisResults.warnings.size(); ++i)
	{
		...
		{
			QString message = QString::asprintf("W%03d: %s", warning.code, warning.text.c_str());
    ...
    QTextCursor&                cursor )
{
    ...
    while ( offset >=0 && offset <= text.length() )
    {
        ...
        if (options & QTextDocument::FindBackward) {
            QRegularExpressionMatchIterator it = expression.globalMatch(text.left(offset));
            if (it.hasNext()) {
                while (it.hasNext()) {
                    match = it.next();
                }
        ...
        if ( options & QTextDocument::FindWholeWords )
        {
            ...
            const int start = idx;
            const int end = start + matchedLength;
    ...
    cursor.setPosition(cursor.position() + matchedLength,QTextCursor::KeepAnchor);
    return true;
}
...
void ScriptTextEditor::onSearchTimeout()
{
	if (!currentWikiSearchSelection.isEmpty())
	{
		...
		else
		{
			...
			if (FFlag::ScriptContextSearchFix && searchTerm.contains(QRegularExpression("[^0-9a-zA-Z]")))
				return;
