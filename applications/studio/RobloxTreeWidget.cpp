...
bool RobloxTreeWidget::event(QEvent* evt)
{
    ...
    else if ( evt->type() == QEvent::ShortcutOverride )
    {
        ...
        if ( !(keyEvent->key() >= Qt::Key_F1 && keyEvent->key() <= Qt::Key_F35) &&
             text.length() == 1 && text[0].isLetterOrNumber() &&
...
bool RobloxTreeWidget::canDragSelect(QMouseEvent * evt)
{
	...
	if (pTreeWidgetItem)
	{
		...
		if ( !itemRect.contains(mousePos) ||
			 (itemRect.contains(mousePos) && (mousePos.x() < (itemRect.bottomLeft().x() + 28 + fontMetrics().horizontalAdvance(pTreeWidgetItem->text(0))))) )
			return false;
	}
