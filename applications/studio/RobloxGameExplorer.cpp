...
typedef std::shared_future<std::string> HttpFuture;
typedef std::shared_future<void> HttpFutureVoid;
...
IdType extractPropertyHelper(std::shared_future<std::string> future, const std::string fieldName)
{
	Variant v;
...
template<class IdType>
static boost::function<IdType()> extractProperty(std::shared_future<std::string> future, const std::string& fieldName)
{
...
{
	QVariant result = QVariant::fromValue(val);
	return result;
}
...
void EntityProperties::setFromJsonFuture(const std::shared_future<std::string>& jsonFuture)
{
...
void UniverseSettings::setFromJsonFuture(const std::shared_future<std::string>& jsonFuture)
{
...
void RobloxGameExplorer::publishNamedAssetsToCurrentSlot()
{
	...
	std::vector<HttpFutureVoid> publishFutures;
	entitySettings[ENTITY_CATEGORY_NamedAssets]->transform<HttpFutureVoid>(
...
{
	QVariant var = QVariant::fromValue(future);
					...
					if (thumbnailDetails.is_initialized() && thumbnailDetails.get())
					{
						...
						if (iter != thumbnailDetails.get()->end())
						{
							...
							if (!thumbnailUrl.empty())
							{
								...
								HttpFuture future = HttpAsync::get(thumbnailUrl, options);
								invokeThumbnailLoadedForImage(this, currentSessionId, newItem->index(), future);
							}
						}
					}
				}
...
void RobloxGameExplorer::afterNamedAssetsFinishedRecursive(QStandardItem* root, QStringList& imagesToReload)
{
	for (int i = 0; i < root->rowCount(); ++i)
	{
		...
		if (EntityProperties* properties = namedImageItem->data(ROLE_EntityProperties).value<EntityProperties*>())
		{
			...
			{
				HttpFuture future = HttpAsync::get(formatUrl("%WEB_BASE%/asset/?id=%ITEM_ID%", -1, assetId.get()));
				invokeThumbnailLoadedForImage(this, currentSessionId, namedImageItem->index(), future);
			}
...
void RobloxGameExplorer::namedAssetsContextMenuHandler(const QPoint& point, EntityProperties* properties)
{
	...
	else if (publishScript && result == publishScript)
	{
		...
		if (mb.exec() == QMessageBox::Yes)
		{
			std::shared_future<void> publishFuture = publishIfThereAreLocalModifications(properties);
			UpdateUIManager::Instance().waitForLongProcess("Publishing LinkedSource...",
				boost::bind(&std::shared_future<void>::get, &publishFuture));
			refreshNamedScriptIcons(currentSessionId);
		}
	}
...
void RobloxGameExplorer::doneEditingUniverseName()
{
	...
	{
		std::promise<std::string> jsonPromise;
		jsonPromise.set_value(newSettings);
		gameSettings.setFromJsonFuture(HttpFuture(jsonPromise.get_future().share()));
	}
...
HttpFutureVoid RobloxGameExplorer::publishIfThereAreLocalModifications(EntityProperties* settings)
{
	...
	{
		HttpFuture scriptPublishFuture = publishScriptAsset(localVersion.get(), settings->get<int>("AssetId").get(),
			currentGameGroupId);
		scriptPublishFuture.get();
		removeFile(getUnpublishedAssetFilePath(name, currentGameId));
		std::promise<void> prom;
		prom.set_value();
		return HttpFutureVoid(prom.get_future());
	}
	else
	{
		std::promise<void> prom;
