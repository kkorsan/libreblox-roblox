...
void CallStackWidget::onCurrentIndexChanged(int index)
{
	...
	if (spScript)
	{
		...
		if (pDebuggerClient)
		{
			...
			{
				bool ok = false;
				int threadID = match.captured(1).toInt(&ok, 16);
				if (ok)
...
void DebuggerToolTipWidget::computeToolTipSize()
{
	...
	// compute total rows height using size hints to avoid calling rowHeight with a QModelIndex
	int rowsHeight = 0;
	if (topLevelItemCount() > 0)
	{
		QModelIndex topIndex = indexFromItem(topLevelItem(0));
		std::function<int(const QModelIndex&)> computeHeight = [&](const QModelIndex& idx) -> int
		{
			if (!idx.isValid())
				return 0;
			int h = sizeHintForIndex(idx).height();
			int childCount = model()->rowCount(idx);
			for (int i = 0; i < childCount; ++i)
				h += computeHeight(model()->index(i, 0, idx));
			return h;
		};
		rowsHeight = computeHeight(topIndex);
	}
	else
	{
		// fallback minimal height if no items
		rowsHeight = sizeHintForRow(0);
		if (rowsHeight <= 0)
			rowsHeight = fontMetrics().height() + 4;
	}

	//calculate available height and width using QScreen (QApplication::desktop() is deprecated/removed)
	const int margin = 5;
	...
	QRect desktopRect = screen ? screen->availableGeometry() : QRect(0, 0, 1920, 1080);
	int maxWidth = desktopRect.right() - globalPos.x() - margin - margin;
	int maxHeight = desktopRect.bottom() - globalPos.y() - margin - margin;
	...
	const int extra = 5;
	setMinimumSize(columnsWidth + extra, rowsHeight);
	setMaximumSize(columnsWidth + extra, rowsHeight);
}
