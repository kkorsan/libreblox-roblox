...
bool BasicFindDialog::getSearchParameters(bool forward,QRegularExpression& outRegExp,QTextDocument::FindFlags& outflags)
{
    ...
    outflags = QTextDocument::FindFlags();
    ...
    if ( !m_pRegExCheckBox->isChecked() )
        outRegExp.setPattern(QRegularExpression::escape(toSearch));
    else
        outRegExp.setPattern(toSearch);

    QRegularExpression::PatternOptions options = m_pCaseCheckBox->isChecked() ? QRegularExpression::NoPatternOption : QRegularExpression::CaseInsensitiveOption;
    outRegExp.setPatternOptions(options);
...
bool ReplaceDialog::isSelectionValid()
{
	...
	if ( !selectedText.isEmpty() )
	{
		...
		if ( !toSearch.isEmpty() )
		{
			...
			if ( !m_pRegExCheckBox->isChecked() )
				reg.setPattern(QRegularExpression::escape(toSearch));

			QRegularExpression::PatternOptions options = m_pCaseCheckBox->isChecked() ? QRegularExpression::NoPatternOption : QRegularExpression::CaseInsensitiveOption;
			reg.setPatternOptions(options);

			//selection and replacement string should be matching
			QRegularExpressionMatch match = reg.match(selectedText);
...
{
	QRegularExpression rx(m_pFindComboBox->lineEdit()->text());

	if (!m_pRegularExpressionButton->isChecked())
		rx.setPattern(QRegularExpression::escape(m_pFindComboBox->lineEdit()->text()));

	QRegularExpression::PatternOptions options = m_pMatchCaseButton->isChecked() ? QRegularExpression::NoPatternOption : QRegularExpression::CaseInsensitiveOption;
	rx.setPatternOptions(options);
