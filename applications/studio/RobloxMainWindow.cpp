...
, m_cookieConstraintCheckDone(NULL)
{
	try
	{
        ...
        if ( showEventArg.isEmpty() )
        {
            ...
            m_splashScreen = new QSplashScreen(QPixmap(":/images/RobloxStudioSplash.png"));
		...
		if (argMap.contains("dumpFlags") && argMap["dumpFlags"] == "TRUE")
			instanceDump();
		...
		QtConcurrent::run([=]() {
			RBX::Http robloxRequest(RobloxSettings::getBaseURL().toStdString() + "/Game/PlaceLauncher.ashx");
			RBX::Http externalRequest("http://www.google.com");
			this->checkInternetConnectionSendCounter(robloxRequest, externalRequest);
		});
        ...
        onPropertyChanged(NULL);
        		...
        		{
        			m_cookieConstraintChecker = new QWebEngineView();
        			m_cookieConstraintCheckDone = new QEventLoop(this);
        			m_cookieConstraintChecker->setAttribute(Qt::WA_DeleteOnClose);
        			connect(m_cookieConstraintChecker, SIGNAL(loadStarted()), m_cookieConstraintChecker, SLOT(hide()));
        			...
        			m_cookieConstraintChecker->load(QUrl(RobloxSettings::getBaseURL()));
		...
		if ( m_splashScreen )
        {
            ...
            if ( RobloxApplicationManager::instance().getApplicationCount() == 1 )
                waitTime = qMax(0,SplashTotalTime - splashTimer.elapsed());
...
#elif __linux__
// TODO: implement linux bootstrapper event
#endif
	return true;
}
...
void RobloxMainWindow::updateRecentFilesUI()
{
	if (isRibbonStyle())
	{
		// SARibbonRecentFileManager* pRecentFileList = findChild<SARibbonRecentFileManager*>("recentFileList");
		// if (pRecentFileList)
...
void RobloxMainWindow::about()
{
	...
	QString copyrightString = QString::asprintf("are trademarks of ROBLOX Corporation %c2014. ", COPYRIGHT_PREFIX);
...
static void dumpVariableEx(const std::string& name, const std::string& value, FastVarType type, VariableKind kind, void* context)
{
	std::ostream* stream = reinterpret_cast<std::ostream*>(context);
	
	// Skip PlaceFilter_ variables as they are internal helper strings
	if (name.compare(0, 12, "PlaceFilter_") == 0)
		return;

	std::string finalName = name;
	
	// Reconstruct prefix based on type and kind
	std::string prefix = "";
	
	if (type & FASTVARTYPE_SYNC)
	{
		prefix = "S";
	}
	else if (type & FASTVARTYPE_DYNAMIC)
	{
		prefix = "D";
	}
	...
	// If the name already has the prefix, don't double it (though macros usually don't include it)
	if (name.find(prefix) == 0)
		finalName = name;
	else
		finalName = prefix + name;

	// ClientAppSettings expects "True"/"False" for booleans
	std::string finalValue = value;
	if (kind == VARKIND_FLAG)
	{
		finalValue = (value == "true") ? "True" : "False";
	}

	*stream << "    \"" << finalName << "\": \"" << finalValue << "\",\n";
}

void RobloxMainWindow::instanceDump()
{
	RBX::StandardOut::singleton()->printf(RBX::MESSAGE_INFO, "Dumping all FastFlags in ClientAppSettings format...");
	
	std::string fileName = (RBX::FileSystem::getUserDirectory(true, RBX::DirAppData) / "FastFlags_Dump.json").string();
	std::ofstream stream(fileName.c_str());
	
	if (stream.is_open())
	{
		stream << "{\n";
		FLog::ForEachVariable(&dumpVariableEx, &stream, FASTVARTYPE_ANY);
		
		// Add some internal info to avoid trailing comma issue and provide context
		stream << "    \"_DumpTime\": \"" << QDateTime::currentDateTime().toString().toStdString() << "\",\n";
		stream << "    \"_AppType\": \"Studio\"\n";
		stream << "}\n";
		stream.close();
		RBX::StandardOut::singleton()->printf(RBX::MESSAGE_INFO, "FastFlags dumped to: %s", fileName.c_str());
	}
	else
	{
		RBX::StandardOut::singleton()->printf(RBX::MESSAGE_ERROR, "Failed to open file for dumping FastFlags: %s", fileName.c_str());
	}
}
...
bool RobloxMainWindow::handleFileOpen(const QString &fileName, IRobloxDoc::RBXDocType type, const QString &script)
{
    ...
    // Initialize RobloxView for all IDE documents after loading/creation
    if (type == IRobloxDoc::IDE)
    {
        RobloxIDEDoc* idedoc = static_cast<RobloxIDEDoc*>(newDoc);
        idedoc->initializeRobloxView();
    }
...
void RobloxMainWindow::trackUserActive()
{
	...
	// Ensure times are valid before computing intervals.
	if (m_lastAnalyticTrackedTime.isNull() || m_lastAnalyticKeepAliveTime.isNull())
		return;

	int msSinceLastTracked = m_lastAnalyticTrackedTime.msecsTo(QTime::currentTime());
	int msSinceLastKeepAlive = m_lastAnalyticKeepAliveTime.msecsTo(QTime::currentTime());

	// If it has been some time (eventIdleTime) since the last event tracked,
	// and the user is not completely idle and some system is posting a keep alive.
	if (msSinceLastTracked > eventIdleTime && msSinceLastKeepAlive < stillAliveTime)
	{
