...
void ChatMessageItem::paint(QPainter* painter, const QStyleOptionViewItem& option)
{
	...
	if (!icon.isNull())
	{
		...
		clipPath.addEllipse(adjustedRect.topLeft().x(), adjustedRect.topLeft().y(), kPlayerIconSize, kPlayerIconSize);
	...
	QFontMetrics subFm(subLabelFont);
	QFontMetrics msgFm(option.font);

	int subLabelImageHeight = subFm.height();
	int messageTextWidth = msgFm.horizontalAdvance(messageText) + 2;
	int subLabelTextWidth = (!sublabelText.isEmpty() ? subFm.horizontalAdvance(sublabelText) : 0) +
	...
	QPainterPath msgPath;
	msgPath.addPolygon(getMessageBoxPolygon(adjustedRect, pointingRight));
	painter->fillPath(msgPath, pointingRight ? QBrush(QColor("#99daff")) : QBrush(QColor("#e3e3e3")));
