...
void UpdateUIManager::updateToolbarTask()
{
	...
	else
	{
		...
		const SARibbonMainWindow* pMainWindow = static_cast<SARibbonMainWindow*>(m_pRobloxMainWindow);
		const int pagesCount = pMainWindow->ribbonBar()->categoryPages().size();
		for (int i = 0; i < pagesCount; i++)
		{
			const QList<SARibbonPanel*> ribbonGroups = pMainWindow->ribbonBar()->categoryByIndex(i)->findChildren<SARibbonPanel*>();
		    for (int k = 0; k < ribbonGroups.count(); k++)
		    {
                SARibbonPanel* group = ribbonGroups[k];

			    const QList<SARibbonGallery*> galleries = group->findChildren<SARibbonGallery*>();
				...
				// SARibbonPanel may not have optionAction getter, skip for now
				// if (group->optionAction() && group->optionAction()->isVisible())
				//	updateAction(group->optionAction());
		    }
		}

		// Update quickaccess bar's actions
		if (pMainWindow->ribbonBar()->quickAccessBar())
			updateActions(pMainWindow->ribbonBar()->quickAccessBar()->actions());
...
void UpdateUIManager::updateActions(const QList<QAction*>& actions)
{
	for (int i = 0; i < actions.size(); ++i)
	{
		...
		if ( action && action->inherits("QWidgetAction") )
		{
...
void UpdateUIManager::updateStatusBar()
{
	...
	QString coresStr;
	coresStr.asprintf("Cores: %.1g", cores);
...
void UpdateUIManager::configureDockWidget(QDockWidget* dockWidget)
{
	...
	{
		if (SARibbonCategory* pPage = m_pRobloxMainWindow->ribbonBar()->findChild<SARibbonCategory*>("View"))
		{
			if (SARibbonPanel* pGroup = pPage->findChild<SARibbonPanel*>("Show"))
				pGroup->addAction(dockWidget->toggleViewAction());
		}
	}
