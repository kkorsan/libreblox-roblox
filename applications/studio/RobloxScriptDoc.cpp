...
bool RobloxScriptDoc::doHandleAction(const QString& actionID, bool isChecked)
{
	...
	if (keySequence.matches(QKeySequence::MoveToNextPage) == QKeySequence::ExactMatch)
	{
		if (m_pTextEdit)
		{
			...
			m_pTextEdit->keyPressEvent(&keyEvent);
		}
...
void RobloxScriptDoc::setMenuVisibility(bool visible)
{
	...
	if (scriptMenu)
	{
		if (mainWindow.isRibbonStyle())
		{
			...
			if (visible)
			{
				slastActivatedRibbonPage = ribbonMainWindow.ribbonBar()->currentIndex();

                ribbonMainWindow.ribbonBar()->setCurrentIndex(scriptMenu->property("index").toInt());
            }
			else if (slastActivatedRibbonPage >= 0)
			{
                ribbonMainWindow.ribbonBar()->setCurrentIndex(slastActivatedRibbonPage);
			}
		}
		else
		{
