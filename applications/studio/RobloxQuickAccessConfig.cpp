...
{
	m_pQuickAccessBar = m_pMainWindow->ribbonBar() ? m_pMainWindow->ribbonBar()->quickAccessBar() : NULL;
...
void RobloxQuickAccessConfig::updateActionVisiblity(QAction* action)
{
	...
	{
		iter->isVisible = true; // Assume visible as SARibbon may not have isActionVisible
		++iter;
	}
}
...
void RobloxQuickAccessConfig::addToQuickAccessBar(QAction* actionToAdd, bool visibility)
{
	...
	// Removed visibility check and set as SARibbon may not support setActionVisible
...
void RobloxQuickAccessConfig::removeFromQuickAccessBar(QAction* actionToRemove)
{
	...
	m_pQuickAccessBar->removeAction(actionToRemove);
}
