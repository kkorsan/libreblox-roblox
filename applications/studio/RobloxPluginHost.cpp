...
long long RobloxPluginHost::m_NextID = 0;
...
{
    long long toolbarID = (long long)tbId;

	RobloxMainWindow& mainWindow = UpdateUIManager::Instance().getMainWindow();

	if (mainWindow.isRibbonStyle() && m_TextActionMap.contains(text.c_str()))
	{
		if (static_cast<int>(toolbarID) == m_TerrainToolbarID)
			return (void*)m_TextActionMap[text.c_str()];
		else if(static_cast<int>(toolbarID) == m_TransformToolbarID)
			return (void*)m_TextActionMap[text.c_str()];
	}

    long long actionID = m_NextID++;

    QMetaObject::invokeMethod(
        this,
        "onCreateButton",
        Qt::QueuedConnection,
        Q_ARG(int,static_cast<int>(toolbarID)),
        Q_ARG(int,actionID),
        Q_ARG(QString,text.c_str()),
        Q_ARG(QString,tooltip.c_str()),
        Q_ARG(QString,QString::fromUtf8(iconFilePath.c_str())) );

    return (void*)(long long)actionID;
}
...
void RobloxPluginHost::setButtonActive(void *bId, bool active)
{
	QMetaObject::invokeMethod(this, "onSetButtonActive", Qt::QueuedConnection,
		Q_ARG(long long, (long long)(bId)), Q_ARG(bool, active));
}
...
void RobloxPluginHost::setButtonIcon_deprecated(void* buttonId, const std::string& iconFilePath)
{
	QMetaObject::invokeMethod(this, "setButtonIconInternal_deprecated", Qt::QueuedConnection,
		Q_ARG(long long, (long long)(buttonId)), Q_ARG(QString, QString(iconFilePath.c_str())));
}
...
void RobloxPluginHost::setButtonIcon(void* buttonId, const std::string& imageContent)
{
	...
	else
	{
		QMetaObject::invokeMethod(this, "setButtonIconInternal", Qt::QueuedConnection,
			Q_ARG(long long, (long long)(buttonId)), Q_ARG(QImage, image));
	}
}
...
void RobloxPluginHost::setButtonIconInternal_deprecated(long long buttonId, QString iconFilePath)
{
	...
	QAction* action = m_ActionMap[buttonId];
...
void RobloxPluginHost::setButtonIconInternal(long long buttonId, QImage image)
{
	if (!m_ActionMap.contains(buttonId))
    {
        // This can happen if it takes a long time to load an icon
...
void RobloxPluginHost::hideToolbars(const std::vector<void*>& toolbars, bool hide)
{
	...
	QList<long long> toolbarIds;
	for ( size_t i = 0 ; i < toolbars.size( ); ++i )
		toolbarIds.push_back((long long)toolbars[i]);
...
void RobloxPluginHost::disableToolbars(const std::vector<void*>& toolbars, bool disable)
{
    ...
    for ( size_t i = 0 ; i < toolbars.size( ); ++i )
    {
        long long toolbarID = (long long)toolbars[i];
        QMetaObject::invokeMethod(
                                  this,
                                  "onEnableToolbar",
                                  Qt::QueuedConnection,
                                  Q_ARG(long long,toolbarID),
                                  Q_ARG(bool,!disable) );
    }
}

void RobloxPluginHost::onShowToolbar(long long toolbarID, bool show)
{
    if ( !m_ToolBarMap.contains(toolbarID) && !m_RibbonGroupMap.contains(toolbarID) )
	{
        ...
        if ( show )
            RBX::StandardOut::singleton()->printf(RBX::MESSAGE_INFO,"Plugin attempting to show toolbar that does not exist - %lli",toolbarID);
        return;
	}
	...
	if ( mainWindow.isRibbonStyle() )
	{
		SARibbonPanel* group = m_RibbonGroupMap[toolbarID];
...
{
	QList<long long> toolbars = toolbarsVariant.value<QList<long long> >();
...
void RobloxPluginHost::onEnableToolbar(long long toolbarID, bool enable)
{
    if ( !m_ToolBarMap.contains(toolbarID) && !m_RibbonGroupMap.contains(toolbarID) )
	{
		// only output an error if we're showing a toolbar that is supposed to exist
        if ( enable )
            RBX::StandardOut::singleton()->printf(RBX::MESSAGE_INFO,"Plugin attempting to disable/enable toolbar that does not exist - %lli",toolbarID);
        return;
	}
    ...
    for ( size_t i = 0 ; i < toolbars.size() ; ++i )
    {
        long long toolbarID = (long long)toolbars[i];
                ...
                if ( mainWindow.isRibbonStyle() )
        		{
        			...
        			qobject_cast<SARibbonCategory*>(group->parentWidget())->removePanel(group);
        		}
		else
		{
            QToolBar* toolbar = m_ToolBarMap[toolbarID];
...
void RobloxPluginHost::handlePluginAction()
{
	...
	if (RobloxIDEDoc::getIsCloudEditSession() && pAction->data().toLongLong() == m_ConvertToSmoothID)
	{
    ...
    {
        long long actionID = pAction->data().toLongLong();
        RobloxDocManager::Instance().getCurrentDoc()->handlePluginAction(m_pNotifier,(void*)(long long)(actionID));
    }
...
void RobloxPluginHost::onCreateToolbar(long long toolbarID,QString name)
{
    	RobloxMainWindow& mainWindow = UpdateUIManager::Instance().getMainWindow();
    	...
    	if ( mainWindow.isRibbonStyle() )
    	{
    		...
    		SARibbonCategory* page = (name == SmoothTerrainToolbarName) ? m_TerrainToolbar : m_PluginsToolbar;

    		SARibbonPanel* group = page->addPanel(name);
...
void RobloxPluginHost::onCreateButton(long long toolbarID,long long actionID,QString text,QString tooltip,QString iconName)
{
     if ( !m_RibbonGroupMap.contains(toolbarID) && !m_ToolBarMap.contains(toolbarID) )
    {
        RBX::StandardOut::singleton()->printf(RBX::MESSAGE_INFO,"Plugin attempting to add button to toolbar that does not exist - %lli, %lli",toolbarID,actionID);
        return;
    }
	...
	if ( mainWindow.isRibbonStyle() )
	{
		SARibbonPanel* pGroup = m_RibbonGroupMap[toolbarID];
    ...
    action->setData(QVariant::fromValue(actionID));
...
void RobloxPluginHost::onSetButtonActive(long long buttonID, bool active)
{
	if ( !m_ActionMap.contains(buttonID) )
	{
		RBX::StandardOut::singleton()->printf(RBX::MESSAGE_INFO,
			"Plugin attempting to set active state for missing button ID - %lli", buttonID);
		return;
	}
...
void RobloxPluginHost::init()
{
	...
	if ( mainWindow.isRibbonStyle() )
	{
		...
		if ( !m_PluginsToolbar )
			m_PluginsToolbar = mainWindow.ribbonBar()->categoryByObjectName(PluginToolbarName);

		if ( !m_TerrainToolbar )
		{
            const QList<SARibbonCategory*> pages = mainWindow.ribbonBar()->categoryPages();
			for ( int i = 0 ; i < pages.size() ; ++i )
			{
                SARibbonCategory* page = pages[i];
                if ( page->objectName() != TerrainToolbarName )
                    continue;

                m_TerrainToolbar = page;

				const QList<SARibbonPanel*> ribbonGroups = page->panelList();
                for (int j = 0; j < ribbonGroups.count(); j++)
			    {
                    SARibbonPanel* group = ribbonGroups[j];
                        ...
                        continue;
                    		...
                    		m_TerrainToolbarID = m_NextID++;
                    					...
                    					for ( int k = 0 ; k < actions.size() ; ++k )
                    					{
                    						...
                    						const long long actionID = m_NextID++;
                    			...
                    			if (SARibbonCategory* page = mainWindow.ribbonBar()->categoryByObjectName("Model"))
                    			{
                    				if (SARibbonPanel* group = page->panelByObjectName("Tools"))
                    				{
...
{
	QMap<long long,QAction*>::iterator iter = m_ActionMap.begin();
...
{
	SARibbonPanel* group = m_RibbonGroupMap[m_TerrainToolbarID];
	if (!group)
		return;
...
QString RobloxPluginHost::getToolbarName(long long id) const
{
    RobloxMainWindow& mainWindow = UpdateUIManager::Instance().getMainWindow();

	if (mainWindow.isRibbonStyle())
	{
		SARibbonPanel* group = m_RibbonGroupMap[id];
...
bool RobloxPluginHost::actionState(const QString &actionID, bool &enableState, bool &checkedState)
{
	...
	QMapIterator<long long,QAction*> iter(m_ActionMap);
