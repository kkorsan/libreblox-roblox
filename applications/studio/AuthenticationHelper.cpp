...
void AuthenticationHelper::authenticateUserAsync(const QString& url, const QString& ticket)
{
	if (!url.isEmpty() && !ticket.isEmpty())
	{
		...
		// Disambiguate overloaded member function by using a lambda
		m_HttpAuthenticationFuture = QtConcurrent::run([this, authenticationUrl]() -> bool {
		...
		m_QtWebkitAuthenticationFuture = QtConcurrent::run([this]() -> bool {
			return this->authenticateQtWebkitSession();
		});
	...
	else
	{
		...
		else
		{
			...
			m_HttpAuthenticationFuture = QtConcurrent::run([this]() -> bool {
				return this->authenticateHttpSession();
			});
			m_HttpAuthenticationFutureWatcher.setFuture(m_HttpAuthenticationFuture);
		}
	}
...
void AuthenticationHelper::authenticateHttpSessionAsync(bool state)
{
	...
	if (state)
	{
		// Use a lambda to avoid overload / pointer-to-member ambiguity with QtConcurrent::run
		m_HttpAuthenticationFuture = QtConcurrent::run([this]() -> bool {
			return this->authenticateHttpSession();
		});
	}
	else
	{
		// Use a lambda to avoid overload / pointer-to-member ambiguity with QtConcurrent::run
		m_HttpAuthenticationFuture = QtConcurrent::run([this]() -> bool {
			return this->deAuthenticateHttpSession();
		});
	}
