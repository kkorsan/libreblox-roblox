...
: SARibbonMainWindow(NULL, SARibbonMainWindowStyleFlag::UseNativeFrame)
, m_LastDirectory(".")
, m_ribbonTheme(SARibbonTheme::RibbonThemeOffice2013)
...
void RobloxRibbonMainWindow::setupRibbonBar()
{
	...
	if (!doc.setContent(data, false, &errorMsg, &errorLine, &errorColumn))
		throw RBX::runtime_error("Failed to parse configuration XML file. Error: %s at line %d, column %d", errorMsg.toStdString().c_str(), errorLine, errorColumn);
	...
	// Configure ribbon for proper panel display to match original QtitanRibbon layout
	ribbonBar()->setRibbonStyle(SARibbonBar::RibbonStyleLooseThreeRow);

	m_bInitialized = true;
}

SARibbonCategory* RobloxRibbonMainWindow::createRibbonPage(const QString& objectName, const QString& pageTitle, int indexPosition)
{
	SARibbonCategory* pRibbonPage = ribbonBar()->categoryByObjectName(objectName);
	if (pRibbonPage)
		return pRibbonPage;

	// Categories should be tacked onto the end of the bar, unless an explicit index has been passed in.
	int insertedPosition = indexPosition >= 0 ? indexPosition : ribbonBar()->categoryPages().size();

	pRibbonPage = ribbonBar()->insertCategoryPage(pageTitle, insertedPosition);
...
SARibbonPanel* RobloxRibbonMainWindow::createRibbonGroup(SARibbonCategory* pParent, const QString& objectName, const QString& groupTitle)
{
	if (!pParent)
		return NULL;

	SARibbonPanel* pRibbonGroup = pParent->panelByObjectName(objectName);
	if (pRibbonGroup)
		return pRibbonGroup;

	QString displayTitle = groupTitle.isEmpty() ? objectName : groupTitle;
	pRibbonGroup = pParent->addPanel(displayTitle);
	RBXASSERT(pRibbonGroup);
	pRibbonGroup->setObjectName(objectName);
	pRibbonGroup->setWindowTitle(displayTitle);
...
void RobloxRibbonMainWindow::parseAndCreateQuickAccessBar(const QDomElement &docElem)
{
	...
	SARibbonQuickAccessBar* pQuickAccessBar = ribbonBar()->quickAccessBar();
	if (!pQuickAccessBar)
		return;

	// SARibbon does not have customizeAction for quick access bar

	// SARibbon doesn't have showQuickAccess method, quick access bar is always visible if it has actions
...
void RobloxRibbonMainWindow::parseAndCreateSystemButton(const QDomElement &docElem)
{
    ...
    // Set application button icon
    if (ribbonBar()->applicationButton())
    {
    	SARibbonApplicationButton* appButton = qobject_cast<SARibbonApplicationButton*>(ribbonBar()->applicationButton());
    	appButton->setIcon(iconLogo);
    	appButton->setText(buttonName);

    	QString tooltip  = getToolTip(systemButtonElement);
    	appButton->setToolTip(tooltip);

    	QDomElement popupElement = systemButtonElement.firstChildElement(elemSystemPopup);
    	if (!popupElement.isNull())
    	{
    		QMenu* popupMenu = new QMenu(this);
    		appButton->setMenu(popupMenu);
    		connect(popupMenu, SIGNAL(aboutToShow()), &UpdateUIManager::Instance(), SLOT(onMenuShow()));

    		parseAndCreateChildren(popupElement, popupMenu);

    		// Add recent files action
    		QAction* recentFilesAction = popupMenu->addAction(tr("Recent Files"));
    		recentFilesAction->setObjectName("recentFileList");
    		// SARibbon doesn't have built-in recent files list, so we'll handle this differently

    		QAction* pActionExit = popupMenu->addAction(tr("Exit"));
    		connect(pActionExit, SIGNAL(triggered()), this, SLOT(close()));

    		//QAction* pActionOption = popupMenu->addAction(tr("Settings"));
...
void RobloxRibbonMainWindow::parseAndCreateRibbonBar(const QDomElement &docElem)
{
	...
	{
		// Add online help button to the right button group
		SARibbonButtonGroupWidget* rightGroup = ribbonBar()->rightButtonGroup();
		if (rightGroup)
		{
			rightGroup->addAction(onlineHelpAction);
			QWidget* buttonWidget = rightGroup->widgetForAction(onlineHelpAction);
			if (QToolButton* toolButton = qobject_cast<QToolButton*>(buttonWidget))
			{
		...
		{
			m_pRibbonMinimizeAction = ribbonBar()->minimumModeAction();
			if (m_pRibbonMinimizeAction)
			{
				...
				connect(ribbonBar(), SIGNAL(ribbonModeChanged(SARibbonBar::RibbonMode)), this, SLOT(handleRibbonMinimizationChanged(SARibbonBar::RibbonMode)));

				// add this to registry
	            RobloxSettings settings;
				ribbonBar()->setMinimumMode(settings.value(settingRibbonMinimized, false).toBool());
			}
		}
...
SARibbonCategory* RobloxRibbonMainWindow::parseAndCreateRibbonPage(const QDomElement& domElement)
{
	...
	SARibbonCategory* pRibbonPage = createRibbonPage(objectName, pageTitle);

	// Check to see if this is a context tab
	QString contextTitle = domElement.attribute(attribContextTitle);
	if (!contextTitle.isEmpty())
	{
		// SARibbon doesn't have context categories in the same way, so we'll skip this for now
		// TODO: Implement context categories if needed
	}
...
SARibbonPanel* RobloxRibbonMainWindow::parseAndCreateRibbonGroup(const QDomElement& domElement, QObject* pParent)
{
	SARibbonCategory* pParentPage = dynamic_cast<SARibbonCategory*>(pParent);
	if (!pParentPage)
		throw RBX::runtime_error("Panel parent must be a SARibbonCategory!");

	QString groupName = getName(domElement);
	QString groupTitle = domElement.attribute(attribText);
	if (groupTitle.isEmpty()) {
		groupTitle = groupName;
	}
	SARibbonPanel* pRibbonGroup = createRibbonGroup(pParentPage, groupName, groupTitle);
	RBXASSERT(pRibbonGroup);

	// Clear any leftover grouped button state for this panel
	// This is needed because the static map persists across function calls
	static QMap<SARibbonPanel*, RibbonGroupedButton*> s_lastGroupedButton;
	s_lastGroupedButton.remove(pRibbonGroup);

	// Handle option buttons if specified
	if (domElement.attribute("options") == "true") {
		// SARibbon uses setOptionAction instead of setOptionButtonVisible
		// We'll create a dummy action for the option button
		QAction* optionAction = new QAction(pRibbonGroup);
		optionAction->setToolTip("Group Options");
		pRibbonGroup->setOptionAction(optionAction);
	}
...
{
	SARibbonPanel* pRibbonPanel = qobject_cast<SARibbonPanel*>(pParent);
	if (!pRibbonPanel)
		throw RBX::runtime_error("Color picker must have a ribbon panel parent element!");
	...
	SARibbonToolButton* pColorButton = new SARibbonToolButton(pRibbonPanel);
	pColorButton->setObjectName(getName(domElement));
	pColorButton->setText(text);
	pColorButton->setIcon(icon);
	pColorButton->setToolTip(getToolTip(domElement));
	...
	// Connect to color pick menu handler
	connect(pColorButton, SIGNAL(clicked()), this, SLOT(handleColorPickMenu()));

	pRibbonPanel->addWidget(static_cast<QWidget*>(pColorButton), SARibbonPanelItem::RowProportion::Large);
...
QMenu* RobloxRibbonMainWindow::parseAndCreateMenuPopupButton(const QDomElement& domElement, QObject* pParent)
{
	SARibbonPanel* pRibbonPanel = qobject_cast<SARibbonPanel*>(pParent);
	if (!pRibbonPanel)
		throw RBX::runtime_error("Menu buttons must have a ribbon panel parent element!");
	...
	QString objectName = getName(domElement);
	ToolButtonProxyMenu* menu = new ToolButtonProxyMenu(objectName, pRibbonPanel);
    ...
    pRibbonPanel->addLargeAction(action);
    ...
    // SARibbonPanel doesn't expose layout items directly, so we need to find the tool button differently
    // This might require adjusting how we access the tool button
    QList<QToolButton*> toolButtons = pRibbonPanel->findChildren<QToolButton*>();
    QToolButton* widgetAction = nullptr;
         ...
         iter++)
    {
        if ((*iter)->objectName() == value)
        {
            ...
            if (menu->toolButton && !menu->toolButton->icon().isNull()) {
...
QMenu* RobloxRibbonMainWindow::parseAndCreateMenu(const QDomElement& domElement, QObject* pParent)
{
	...
	QMenu* pMenu = new QMenu(text, this);
	QAction* pMenuAction = pRibbonPanel->addAction(text, icon, getPopupStyle(domElement), SARibbonPanelItem::Large);
	pMenuAction->setMenu(pMenu);

	connect(pMenu, SIGNAL(aboutToShow()), &UpdateUIManager::Instance(), SLOT(onMenuShow()));

	RBXASSERT(pMenu);
	RBXASSERT(pMenuAction);

	QString objectName = getName(domElement);
	pMenu->setObjectName(objectName);
	pMenuAction->setToolTip(getToolTip(domElement));
	pMenuAction->setObjectName(objectName);
	...
	if ( !domElement.attribute(attribCheckable).isEmpty() )
		pMenuAction->setCheckable(domElement.attribute(attribCheckable) == "true");
...
SARibbonGallery* RobloxRibbonMainWindow::parseAndCreateRibbonGallery(const QDomElement& domElement, QObject* pParent)
{
	SARibbonPanel* pRibbonPanel = qobject_cast<SARibbonPanel*>(pParent);
	if (!pRibbonPanel)
		throw RBX::runtime_error("Gallery parent must be a ribbon panel!");

	SARibbonGallery* pGallery = new SARibbonGallery;
	pGallery->setObjectName(getName(domElement));

    SARibbonGalleryGroup* pGalleryGroup = new SARibbonGalleryGroup;
    ...
    pGallery->addGalleryGroup(pGalleryGroup);
	...
	RBXASSERT(pGalleryGroup->model()->rowCount() > 0);
	...
	int galleryHeight = (qCeil(pGalleryGroup->model()->rowCount()/(float)numColumns)*itemSize.height());
	...
	if (!domElement.attribute(attribPopup).isEmpty() && domElement.attribute(attribPopup).toLower() == "false")
	{
		...
		pRibbonPanel->addWidget(static_cast<QWidget*>(pGallery), SARibbonPanelItem::RowProportion::Large);
	}
	else
	{
		pGallery->setBaseSize(QSize(galleryWidth+4, galleryHeight+2));

		QMenu* pPopupMenu = new QMenu(this);

		pRibbonPanel->addLargeMenu(pPopupMenu, QToolButton::InstantPopup);
		QAction* pAction = pPopupMenu->menuAction();
		pAction->setObjectName(getName(domElement));
	}

	connect(pGallery, SIGNAL(itemClicked(SARibbonGalleryItem*)), this, SLOT(handleBasicObjectsGalleryItemClicked(SARibbonGalleryItem*)));

	return pGallery;
}

SARibbonGalleryItem* RobloxRibbonMainWindow::parseAndCreateGalleryItem(const QDomElement& domElement, QObject* pParent)
{
	SARibbonGalleryGroup* pGalleryGroup = qobject_cast<SARibbonGalleryGroup*>(pParent);
	if (!pGalleryGroup)
		throw RBX::runtime_error("galleryitem must be beneath a gallery group");

	SARibbonGalleryItem* pGalleryItem = new StudioGalleryItem;

	pGalleryItem->setText(getText(domElement));
	pGalleryItem->setToolTip(getToolTip(domElement));
	pGalleryItem->setData(Qt::SizeHintRole, pGalleryGroup->gridSize());
	...
	pGalleryGroup->addItem(pGalleryItem);
...
{
	SARibbonPanel* pGroup = qobject_cast<SARibbonPanel*>(pParent);
	...
	if (pGroup)
		dummyActionGroup.setProperty("ribbonPanel", QVariant::fromValue((quintptr)pGroup));
	//now recursively create child elements
...
QButtonGroup* RobloxRibbonMainWindow::parseAnCreateButtonGroup(const QDomElement& domElement, QObject* pParent)
{
	SARibbonPanel* pRibbonPanel = qobject_cast<SARibbonPanel*>(pParent);
	if (!pRibbonPanel)
		throw RBX::runtime_error("buttongroup must have a ribbon panel parent element!");
	...
	// add created buttons to ribbon panel
	...
	if (buttons.size() > 0)
	{
		for (int ii = 0; ii < buttons.size(); ++ii)
		{
			...
			// add widget to ribbon panel
			pRibbonPanel->addWidget(static_cast<QWidget*>(buttons.at(ii)), SARibbonPanelItem::RowProportion::Large);
		}
...
{
	SARibbonPanel* pRibbonGroup = qobject_cast<SARibbonPanel*>(pParent);
	if (!pRibbonGroup)
	...
	pRibbonGroup->addLargeMenu(pMenu, QToolButton::MenuButtonPopup);
...
QAction* RobloxRibbonMainWindow::parseAndCreateSeparator(const QDomElement& domElement, QObject* pParent)
{
	...
	if (SARibbonPanel* pParentGroup = qobject_cast<SARibbonPanel*>(pParent))
...
QRadioButton* RobloxRibbonMainWindow::parseAndCreateRadioButton(const QDomElement& domElement, QObject* pParent)
{
	...
	if (!pRibbonPanel && !pButtonGroup)
		throw RBX::runtime_error("radio buttons must be beneath a button/ribbon panel");
	...
	if (pRibbonPanel)
		addWidgetToRibbonGroup(pRibbonPanel, pRadioButton);
	else
...
QCheckBox* RobloxRibbonMainWindow::parseAndCreateCheckBox(const QDomElement& domElement, QObject* pParent, bool actionMode)
{
	SARibbonPanel* pRibbonPanel = qobject_cast<SARibbonPanel*>(pParent);
	if (!pRibbonPanel)
		throw RBX::runtime_error("checkbox must be beneath a ribbon panel");
	...
	// add to ribbon panel
	addWidgetToRibbonGroup(pRibbonPanel, pCheckBox);
...
QComboBox* RobloxRibbonMainWindow::parseAndCreateComboBox(const QDomElement& domElement, QObject* pParent)
{
	SARibbonPanel* pRibbonPanel = qobject_cast<SARibbonPanel*>(pParent);
	if (!pRibbonPanel)
		throw RBX::runtime_error("combobox must be beneath a ribbon panel");

	QString objectName = getName(domElement);
	if (objectName.isEmpty())
		throw RBX::runtime_error("combobox name element can not be empty!");
	...
	// add to ribbon panel
	addWidgetToRibbonGroup(pRibbonPanel, pComboBox);
...
QMenu* RobloxRibbonMainWindow::parseAndCreateGroupOptionsMenu(const QDomElement& domElement, QObject* pParent)
{
	SARibbonPanel* pRibbonPanel = qobject_cast<SARibbonPanel*>(pParent);
	if (!pRibbonPanel)
		throw RBX::runtime_error("optionsMenu must be beneath a ribbon panel");

	QMenu* pMenu = new QMenu(pRibbonPanel);
	pMenu->setObjectName(QString("__%1Panel-Menu_").arg(pRibbonPanel->objectName()));
	parseAndCreateChildren(domElement, pMenu);
	...
	if (!pMenu->isEmpty())
	{
		...
		QAction* optionButtonAction = new QAction(this);
		pRibbonPanel->setOptionAction(optionButtonAction);
...
void RobloxRibbonMainWindow::showGroupOptionsMenu()
{
	...
	if (pSender)
	{
		...
		if (pWidget)
		{
			QMenu* pMenu = pWidget->findChild<QMenu*>(QString("__%1Panel-Menu_").arg(pWidget->objectName()));
			if (pMenu)
...
QSize RobloxRibbonMainWindow::getSize(const QDomElement& domElement, const QSize& defaultSize) const
{
	...
	if (width > 0)
		...
		size.setWidth(qRound(size.width() * qApp->devicePixelRatio()));

    int height = domElement.attribute(attribIconHeight).toInt();
	if (height > 0)
		size.setHeight(height);
	else
		size.setHeight(qRound(size.height() * qApp->devicePixelRatio()));
...
void RobloxRibbonMainWindow::addWidgetToRibbonGroup(SARibbonPanel* pRibbonPanel, QWidget* pWidget)
{
	if (!pRibbonPanel || !pWidget)
		return;

	QAction* pAction = pRibbonPanel->addWidget(static_cast<QWidget*>(pWidget), SARibbonPanelItem::RowProportion::Large);
...
{
	// SARibbon: panel-level handling with per-run small-action grouping and grouped buttons
	if (SARibbonPanel* pRibbonPanel = qobject_cast<SARibbonPanel*>(pParent))
	{
		// Determine "small" versus "large" action based on XML attributes/styles.
		QString styleAttr = domElement.attribute(attribStyle).toLower();
		bool isSmallStyle = (styleAttr == "textbesideicon" || styleAttr == "icononly");

		// Heuristic: small if explicit small style OR explicit small icon size
		if (!isSmallStyle)
		{
			int iconWidth = domElement.attribute(attribIconWidth).toInt();
			if (iconWidth > 0 && iconWidth <= 16)
				isSmallStyle = true;
		}

		// static map to remember the last created RibbonButtonGroup per panel.
		// This allows us to group runs of consecutive small actions into the same widget.
		static QMap<SARibbonPanel*, RibbonButtonGroup*> s_lastButtonGroup;

		// static map to remember RibbonGroupedButton per panel for large + small action patterns
		static QMap<SARibbonPanel*, RibbonGroupedButton*> s_lastGroupedButton;

		if (isSmallStyle)
		{
			// Check if we have an active RibbonGroupedButton to add to
			RibbonGroupedButton* pGroupedButton = s_lastGroupedButton.value(pRibbonPanel, nullptr);
			if (pGroupedButton)
			{
				// Add to the grouped button instead of creating a SARibbonButtonGroupWidget
				pGroupedButton->addSideAction(pAction);
				return;
			}

			// Check previous sibling in the DOM to determine whether to continue an existing run.
			bool prevWasSmall = false;
			QDomElement prev = domElement.previousSiblingElement();
			// Skip invisible separators when determining grouping runs
			while (!prev.isNull() && prev.tagName().toLower() == elemAddSeparator)
			{
				QString vis = prev.attribute(attribVisible).toLower();
				if (vis == "false")
				{
			...
			if (!prev.isNull())
			{
				QString prevTag = prev.tagName().toLower();
				if (prevTag == elemAddAction)
				{
					QString prevStyle = prev.attribute(attribStyle).toLower();
					prevWasSmall = (prevStyle == "textbesideicon" || prevStyle == "icononly");
					if (!prevWasSmall)
					{
						int prevWidth = prev.attribute(attribIconWidth).toInt();
						if (prevWidth > 0 && prevWidth <= 16)
							prevWasSmall = true;
					}
				}
			}

			RibbonButtonGroup* pButtonGroup = nullptr;
			if (prevWasSmall)
			{
				pButtonGroup = s_lastButtonGroup.value(pRibbonPanel, nullptr);
			}

			// If we didn't find an existing group to continue, create a new one.
			if (!pButtonGroup)
			{
				pButtonGroup = new RibbonButtonGroup(pRibbonPanel);
				pButtonGroup->setObjectName(QString("__%1_ButtonGroup_%2").arg(pRibbonPanel->objectName()).arg(reinterpret_cast<quintptr>(pButtonGroup)));
				// Add the group widget to the ribbon panel as a large-row widget to give more space for the grid.
				pRibbonPanel->addWidget(static_cast<QWidget*>(pButtonGroup), SARibbonPanelItem::RowProportion::Large);
				s_lastButtonGroup.insert(pRibbonPanel, pButtonGroup);
			}

			// Add the action into the current button group.
			pButtonGroup->addAction(pAction);
		}
		else
		{
			// Clear any outstanding small-group run for this panel (large/separator breaks the run).
			s_lastButtonGroup.remove(pRibbonPanel);

			// For large actions, check if we should create a RibbonGroupedButton
			if (styleAttr == "textundericon")
			{
				// Check if the next sibling is a small action - if so, create a grouped button
				QDomElement next = domElement.nextSiblingElement();
				// Skip invisible separators when looking ahead for grouping
				while (!next.isNull() && next.tagName().toLower() == elemAddSeparator)
				{
					QString vis = next.attribute(attribVisible).toLower();
					if (vis == "false")
					{
				...
				if (!next.isNull() && next.tagName().toLower() == elemAddAction)
				{
					QString nextStyle = next.attribute(attribStyle).toLower();
					nextIsSmall = (nextStyle == "textbesideicon" || nextStyle == "icononly");
					if (!nextIsSmall)
					{
						int nextWidth = next.attribute(attribIconWidth).toInt();
						if (nextWidth > 0 && nextWidth <= 16)
							nextIsSmall = true;
					}
				}
				...
				if (nextIsSmall)
				{
					// Create a RibbonGroupedButton and add the large action to it
					RibbonGroupedButton* pGroupedButton = new RibbonGroupedButton(pRibbonPanel);
					pGroupedButton->setObjectName(QString("__%1_GroupedButton_%2").arg(pRibbonPanel->objectName()).arg(reinterpret_cast<quintptr>(pGroupedButton)));
					pGroupedButton->addMainAction(pAction);
					// Add the grouped button widget to the ribbon panel as a large widget
					pRibbonPanel->addWidget(static_cast<QWidget*>(pGroupedButton), SARibbonPanelItem::RowProportion::Large);
					s_lastGroupedButton.insert(pRibbonPanel, pGroupedButton);
					return;
				}
				else
				{
					// Clear any grouped button since we're not using it
					s_lastGroupedButton.remove(pRibbonPanel);
					// Add as regular large action
					pRibbonPanel->addAction(pAction, SARibbonPanelItem::RowProportion::Large);
				}
			}
			else if (styleAttr == "textbesideicon")
			{
				// Clear any grouped button since we're not using it for explicit small
				s_lastGroupedButton.remove(pRibbonPanel);
				// explicit small presentation
				pRibbonPanel->addAction(pAction, SARibbonPanelItem::RowProportion::Small);
			}
			else
			{
				// Clear any grouped button
				s_lastGroupedButton.remove(pRibbonPanel);
				// default to large presentation for unknown/other styles
				pRibbonPanel->addAction(pAction, SARibbonPanelItem::RowProportion::Large);
			}
		}
	}
	...
	else if (QActionGroup* pParentActionGroup = qobject_cast<QActionGroup*>(pParent))
	{
		...
		if (quintptr panelPtr = pParentActionGroup->property("ribbonPanel").value<quintptr>())
		{
			if (SARibbonPanel* panel = reinterpret_cast<SARibbonPanel*>(panelPtr))
			{
				addActionToParent(pAction, panel, domElement);
			}
		}
	}
	else
...
void RobloxRibbonMainWindow::handleActionSelected(QAction* pAction)
{
    ...
    if (pMenu->toolButton && !pMenu->toolButton->icon().isNull() && pMenu->toolButton->defaultAction() && !pMenu->toolButton->icon().availableSizes().isEmpty())
    {
        QPixmap pixmap(iconPath);
        if (!pixmap.isNull()) {
            // Removed setIcon call to prevent crashes - icon updating may not be necessary
            // pMenu->toolButton->defaultAction()->setIcon(QIcon(pixmap));
        } else {
            RBX::StandardOut::singleton()->printf(RBX::MESSAGE_WARNING, "Failed to load icon: %s", qPrintable(iconPath));
        }
    }
...
void RobloxRibbonMainWindow::handleBasicObjectsGalleryItemClicked(SARibbonGalleryItem* item)
{
	SARibbonGallery* pGallery = qobject_cast<SARibbonGallery*>(sender());
	if (!pGallery)
		return;

	// Find the gallery group containing this item and select it
	SARibbonGalleryGroup* currentGroup = pGallery->currentViewGroup();
	if (currentGroup && currentGroup->groupModel()) {
		SARibbonGalleryGroupModel* model = currentGroup->groupModel();
		for (int i = 0; i < model->rowCount(QModelIndex()); ++i) {
			if (model->at(i) == item) {
				currentGroup->selectByIndex(i);
				break;
			}
		}
	}
...
void RobloxRibbonMainWindow::handleRibbonMinimizationChanged(SARibbonBar::RibbonMode mode)
{
    bool minimized = (mode == SARibbonBar::RibbonMode::MinimumRibbonMode);
    if (m_pRibbonMinimizeAction)
    {
...
{
	ribbonBar()->setMinimumMode(!ribbonBar()->isMinimumMode());
}

void RobloxRibbonMainWindow::handleStyleChange(QAction* pAction)
{
	SARibbonTheme themeId = SARibbonTheme::RibbonThemeOffice2013;
	if (pAction->objectName() == "Office2007Blue")
		themeId = SARibbonTheme::RibbonThemeOffice2016Blue;
		...
		themeId = SARibbonTheme::RibbonThemeWindows7;
		...
		themeId = SARibbonTheme::RibbonThemeOffice2013;
...
void RobloxRibbonMainWindow::initialize()
{
	...
	if (!loadedFontFamilies.empty()) {
		...
		// Fallback to a system font if custom font fails
		m_ribbonFont = QFont("Arial", 9);
		RBX::StandardOut::singleton()->printf(RBX::MESSAGE_WARNING, "Failed to load SourceSansPro font; using Arial fallback.");
	}

	// Adjust font size for high DPI displays
	qreal dpiScale = qApp->devicePixelRatio();
	if (dpiScale > 1.0) {
		int extraSize = qRound((dpiScale - 1.0) * 3); // Increase font size for better readability on high DPI
		m_ribbonFont.setPointSize(m_ribbonFont.pointSize() + extraSize);
	}
    ...
    // MAYBE we have to do it here
    setRibbonBar(new SARibbonBar(this));

    // Set ribbon theme
	setRibbonTheme(m_ribbonTheme);

	//  ribbon bar object is created automatically by SARibbonMainWindow
	...
	if (font == m_ribbonFont)
	{
		...
		for (int ii = 0; ii < ribbonWidgets.count(); ii++)
		{
			if (ribbonWidgets[ii] && ribbonWidgets[ii]->metaObject())
			{
                ...
                ribbonWidgets[ii]->setFont(ribbonWidgets[ii]->metaObject()->className() == QString("SARibbonTabBar") ? smallerFont : font);
#else
                if (ribbonWidgets[ii]->metaObject()->className() == QString("SARibbonTabBar"))
                    ribbonWidgets[ii]->setFont(smallerFont);
#endif
			}
		}

		// SARibbon doesn't have system button in the same way, so skip this part
	}
	else
	{
...
{
	SARibbonTheme themeId = ribbonTheme();
...
void RobloxRibbonMainWindow::setTheme(SARibbonTheme themeId)
{
	setRibbonTheme(themeId);
	m_ribbonTheme = themeId;
}
