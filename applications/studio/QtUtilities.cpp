...
namespace QtUtilities
{
    ...
    QByteArray getResourceFileBytes(const QString& resourceName)
    {
        ...
        RBXASSERT(resource.isValid());
        RBXASSERT(resource.data());
        RBXASSERT(resource.size() > 0);

        if (!resource.isValid() || !resource.data() || resource.size() == 0)
            return QByteArray();

        QByteArray data(reinterpret_cast<const char*>(resource.data()), static_cast<int>(resource.size()));

        // Try to uncompress if the data was produced by qCompress().
        // Older Qt versions do not provide an isCompressed() API on QResource,
        // so attempt qUncompress and fall back if it fails.
