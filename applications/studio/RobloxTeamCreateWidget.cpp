...
: m_GroupId(groupId)
{
	...
	auto future = RBX::HttpAsync::get(str.toStdString());
	future.get();
	invokeGroupDataLoaded(QPointer<GroupItem>(this), future);
}
...
void GroupItem::groupDataLoaded(RBX::HttpFuture future)
{
	...
	{
		auto thumbnailFuture = RBX::HttpAsync::get(thumnailUri.get());
		thumbnailFuture.get();
		invokeGroupThumbnailLoaded(QPointer<GroupItem>(this), thumbnailFuture);
	}
}
...
void RobloxTeamCreateWidget::onPlaceIdChanged(const RBX::Reflection::PropertyDescriptor* desc)
{
	...
	auto future = RBX::HttpAsync::get(canManageUrl.toStdString());
	future.get();
	updatePlaceManageStatus(future);
}
...
void RobloxTeamCreateWidget::updatePlaceManageStatus(RBX::HttpFuture future)
{
	...
	auto httpFuture = RBX::HttpAsync::get(url.toStdString());
	httpFuture.get();
	updatePlaceDetails(httpFuture);
}
...
void FriendsInfoLoader::loadPage(int pageToLoad)
{
	...
	auto future = RBX::HttpAsync::get(str.toStdString());
	future.get();
	invokeFriendsDataLoaded(QPointer<FriendsInfoLoader>(this), pageToLoad, future);
}
...
void CreatorsListLoader::loadPage(int pageToLoad)
{
	...
	auto future = RBX::HttpAsync::get(str.toStdString());
	future.get();
	invokeCreatorsListLoaded(QPointer<CreatorsListLoader>(this), pageToLoad, future);
}
