...
// ... (existing code, not showing all context to save tokens, I will target the end of ProcessUserInputMessage and add handleSdlEvent after it)

// Actually, I will insert it after PostUserInputMessage
void UserInput::handleSdlEvent(const SDL_Event& event)
{
    if (sdlGameController)
    {
        sdlGameController->handleEvent(event);
    }
}
...
void UserInput::postProcessUserInput(bool cursorMoved, bool leftMouseUp, RBX::Vector2 wrapMouseDelta, RBX::Vector2 mouseDelta)
{
    ...
    {
        // Fallback: If not in explicit mouse pan mode, still allow right-click to lock cursor for panning
        if (rightMouseDown)
        {
            userInputService->setMouseWrapMode(RBX::UserInputService::WRAP_CENTER);
        }
        else if (userInputService->getMouseWrapMode() == RBX::UserInputService::WRAP_CENTER)
        {
            userInputService->setMouseWrapMode(RBX::UserInputService::WRAP_AUTO);
        }
    }
...
{
    return ( movementKeysDown() || keyDownInternal(RBX::KC_Q) || keyDownInternal(RBX::KC_E) );
}
...
void UserInput::doWrapMouse(const G3D::Vector2& delta, G3D::Vector2& wrapMouseDelta)
{
	...
	case RBX::UserInputService::WRAP_AUTO:
		{
			// printf("WRAP_AUTO, RMB=%d\n", rightMouseDown);
			if (movementKeysDown() || isMouseCaptured || rightMouseDown) {		// 1. If movement keys are down OR dragging (LMB or RMB) - keep the mouse in the window
