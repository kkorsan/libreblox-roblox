...
QPoint CustomToolButton::getPopupLocation(const QSize &pickerFrameSize)
{
	...
	// Determine a global point near the bottom of the button to identify the screen.
	QPoint globalBottomLeft = mapToGlobal(rect.bottomLeft());
	QPoint globalTopLeft = mapToGlobal(rect.topLeft());

	// Find the screen that contains the button (fallback to primary screen).
	QScreen* screen = QGuiApplication::screenAt(globalBottomLeft);
	if (!screen)
		screen = QGuiApplication::primaryScreen();

	QRect screenGeom = screen ? screen->availableGeometry() : QRect(0, 0, 1024, 768);

	// Prefer showing popup below the button when it fits, otherwise show it above.
	if (globalBottomLeft.y() + pickerFrameSize.height() <= screenGeom.y() + screenGeom.height())
		location = globalBottomLeft;
	else
		location = QPoint(globalTopLeft.x(), globalTopLeft.y() - pickerFrameSize.height());

	// Clamp X to the visible screen area.
	location.setX(qMax(screenGeom.left(), qMin(location.x(), screenGeom.right() - pickerFrameSize.width())));
	// Small vertical adjustment to avoid overlap with button border.
...
void SignalConnector::genericSlotArg1(int index)
{
	QComboBox* combo = qobject_cast<QComboBox*>(sender());
	if (combo) {
		QString text = combo->itemText(index);
		m_pPropertyItem->onEvent(sender(), text);
	}
}
...
{
	QScreen* screen = QGuiApplication::screenAt(location);
	if (!screen)
		screen = QGuiApplication::primaryScreen();

	QRect monitorGeometry = screen ? screen->availableGeometry() : QRect(0, 0, 1024, 768);
	...
	int x = qMax(minLocation.x(), qMin(location.x(), maxLocation.x()));
	int y = qMax(minLocation.y(), qMin(location.y(), maxLocation.y()));
...
void PrimaryPartSelectionEditor::updateCursor(const char* cursorImage)
{
	...
	if (!pixmap.isNull())
	{
		...
		QApplication::setOverrideCursor(pixmapCursor);
	}
}
