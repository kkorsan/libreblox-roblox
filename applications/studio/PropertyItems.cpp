...
public:
	...
	void setEditorData(QWidget *editor)
	{
		...
		pSignalConnector->connect(pComboBox, SIGNAL(activated(int)), SLOT(genericSlotArg1(int)));
	}
...
void PropertyItem::init(const QString &itemName)
{
	...
	if (m_pPropertyDescriptor)
	{
        ...
        if ( metadata && !metadata->description.empty() )
			setToolTip(0,QtUtilities::wrapText(QString::fromStdString(metadata->description).replace(QRegularExpression("<a href.*(\\/a>|\\/>)"), ""),80));
	}
...
QPoint PropertyItem::computePopupLocation(const QSize &pickerFrameSize)
{
	...
	QRect rect = getTreeWidget()->visualItemRect(this);
	QWidget* tree = getTreeWidget();

	// Prefer the bottom-left of the item as the anchor
	QPoint globalBottomLeft = tree->mapToGlobal(rect.bottomLeft());
	QPoint globalTopLeft = tree->mapToGlobal(rect.topLeft());

	// Find the screen that contains the anchor point; fall back to primary screen
	...
	// Prefer showing below the item if there is space, otherwise show above
	if (globalBottomLeft.y() + pickerFrameSize.height() <= screenGeom.bottom())
		location = globalBottomLeft;
	else
		location = globalTopLeft - QPoint(0, pickerFrameSize.height());

	// Clamp X to the available screen area and apply offsets similar to original behavior
	int clampedX = qMax(screenGeom.left(), qMin(location.x(), screenGeom.right() - pickerFrameSize.width())) - 18;
	int adjustedY = location.y() + 24;

	location.setX(clampedX);
	location.setY(adjustedY);
