	...
	: QDialog(Parent,Qt::WindowTitleHint | Qt::WindowSystemMenuHint)
{
	...
	// Use primaryScreen() instead of the removed QApplication::desktop()
	auto screen = QApplication::primaryScreen();
	int dpi = screen ? static_cast<int>(screen->physicalDotsPerInchX()) : 96;
	m_UI.dpiValue->setText(QString("%1 DPI").arg(dpi));
...
void AddEmulationDeviceDialog::submitDevice()
{
	...
	StudioDeviceEmulator::EmulationDevice device(m_UI.lineEditName->text(), dpi, m_UI.spinBoxWidth->value(), m_UI.spinBoxHeight->value(), m_UI.checkBoxMobile->checkState());
