...
, m_CurrentColorCount(0)
{
	...
	// Ensure we have a valid data model before asserting or accessing it.
	if (m_pDataModel)
	{
...
void PlayersDataManager::getPlayerName(int playerId, QObject* resultListener, const char* callbackName)
{
	...
	{
		QMetaObject::invokeMethod(resultListener, callbackName, Qt::AutoConnection,
			Q_ARG(int, playerId), Q_ARG(QString, m_PlayersInfo.value(playerId)));
	}
	else
	{
		QString str = QString("%1/Users/%2").arg(RobloxSettings::getApiBaseURL()).arg(playerId);

		RBX::HttpFuture future = RBX::HttpAsync::get(str.toStdString());
		invokePlayerDataLoaded(QPointer<PlayersDataManager>(this), future);
...
, m_bFinishedLoading(false)
{
	...
	RBX::HttpFuture future = RBX::HttpAsync::get(str.toStdString());
	invokeHeadshotThumbnailDataLoaded(QPointer<AvatarLoader>(this), playerId, future);
}

void AvatarLoader::headshotThumbnailDataLoaded(int playerId, RBX::HttpFuture future)
{
	boost::optional<std::string> thumbnailUri;

	try
	{
		std::string result = future.get();
		boost::shared_ptr<const RBX::Reflection::ValueTable> table;
		if (!result.empty() && RBX::WebParser::parseJSONTable(result, table) && table->size() > 0)
		{
			...
			{
				RBX::Reflection::ValueTable::const_iterator thumbnailUrlIter = table->find("Url");
				if (thumbnailUrlIter != table->end() && thumbnailUrlIter->second.isString())
					thumbnailUri = thumbnailUrlIter->second.get<std::string>();
			}
			else
			{
				...
				if (retryUrlIter != table->end() && retryUrlIter->second.isString())
					thumbnailUri = retryUrlIter->second.get<std::string>();
			}
		}
	}
	catch (const RBX::base_exception& )
	{
		// ignore and do not attempt to load thumbnail
	}

	// TODO: in case of error shall we again try to reload avatar?
	if (thumbnailUri.is_initialized() && !thumbnailUri.get().empty())
	{
		RBX::HttpOptions options;
		options.setExternal(true);
		// RBX::HttpAsync::get returns an RBX::HttpFuture (shared_future) which does not have .then()
		// so call get and schedule the headshotThumbnailLoaded invocation using invokeHeadshotThumbnailLoaded
		RBX::HttpFuture thumbFuture = RBX::HttpAsync::get(thumbnailUri.get(), options);
		invokeHeadshotThumbnailLoaded(QPointer<AvatarLoader>(this), playerId, thumbFuture);
	}
}
