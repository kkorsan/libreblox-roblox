...
JNIEXPORT jstring JNICALL Java_com_roblox_client_ActivityCurlTest_nativeGetURL(JNIEnv *jenv, jclass obj, jstring jsURL)
{
    ...
    {
        std::string response;
        // Treat as external if URL is not a subdomain of roblox.com, robloxlabs.com,
        // or the configured base URL host (so test/prod base host is honored).
        RBX::Url parsed = RBX::Url::fromString(url);

        bool isInternal = parsed.isSubdomainOf("roblox.com") || parsed.isSubdomainOf("robloxlabs.com");

        const std::string baseUrl = ::GetBaseURL();
        if (!baseUrl.empty())
        {
            RBX::Url baseParsed = RBX::Url::fromString(baseUrl);
            const std::string baseHost = baseParsed.host();
        ...
        bool externalRequest = !isInternal;
