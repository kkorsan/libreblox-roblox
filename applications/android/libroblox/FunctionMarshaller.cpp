...
FunctionMarshaller::FunctionMarshaller(uint64_t threadID)
	:refCount(0)
{
	this->threadID = threadID;
	RBX::JNI::createAppEventWindow();
}
...
FunctionMarshaller* FunctionMarshaller::GetWindow()
{
	...
	std::map<uint64_t, FunctionMarshaller*>::iterator find = staticData().windows.find(threadID);
...
{
	for (std::map<uint64_t, FunctionMarshaller*>::iterator iter = windows.begin(); iter != windows.end(); ++iter)
