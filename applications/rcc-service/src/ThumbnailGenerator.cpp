...
{
    // Try to create a dummy X11 window and GL context. If there's no X display
    // (common on headless servers), provide a helpful error and retry a few times.
    void* dummyWindowHandle = nullptr;
    DummyWindowHandle* dw = nullptr;
	...
	while (true)
	{
		try
		{
			createDummyWindow(dummyWindowHandle, 1, 1);
			dw = static_cast<DummyWindowHandle*>(dummyWindowHandle);

			OSContext dummyContext;
			dummyContext.window = reinterpret_cast<void*>(dw->window);
			dummyContext.display = dw->display;
			dummyContext.width = dw->width;
			dummyContext.height = dw->height;
		...
		{
			RBX::StandardOut::singleton()->printf(RBX::MESSAGE_ERROR, "ThumbnailGenerator: Failed to initialize GL context: %s", e.what());
			RBX::StandardOut::singleton()->print(RBX::MESSAGE_ERROR, "If you're running on a headless Linux server, try running under Xvfb (e.g. `xvfb-run -a <command>`) or set the DISPLAY environment variable.");
			if (dummyWindowHandle)
			{
				destroyDummyWindow(dummyWindowHandle);
			...
			if (++retry >= maxRetries)
			{
				RBX::StandardOut::singleton()->print(RBX::MESSAGE_ERROR, "ThumbnailGenerator: giving up after multiple attempts to initialize the GL context.");
				return;
			}

			// wait a bit before retrying
			std::this_thread::sleep_for(std::chrono::seconds(5));
			RBX::StandardOut::singleton()->printf(RBX::MESSAGE_INFO, "ThumbnailGenerator: retrying initialize (%d/%d)", retry, maxRetries);
		}
	}
	...
	while (true)
	{
		...
		while (gThumbRenderQueue.pop_if_present(request))
		{
			...
			try
			{
				...
				DataModel::scoped_write_transfer writeTransfer(static_cast<RBX::Instance*>(request.generator));

                // filetype==OBJ
                if (strcasecmp(request.fileType.c_str(), "obj") == 0)
                {
                    request.generator->exportScene(dummyView.get(), request.strOutput);
                }
				else
				{
					request.generator->renderThumb(dummyView.get(), reinterpret_cast<void*>(dw->window), request.fileType, request.cx, request.cy, request.hideSky, request.crop, request.strOutput);
				}
...
void ThumbnailGenerator::configureCaches()
{
	if(RBX::ContentProvider* contentProvider = RBX::ServiceProvider::create<RBX::ContentProvider>(this)){
		if (contentProvider->getBaseUrl().empty()) {
			contentProvider->setBaseUrl(::GetBaseURL());
		}
...
{
	__sync_add_and_fetch(&totalCount, 1);
	try
	{
		ReadAccessKey();
