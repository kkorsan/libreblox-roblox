...
RenderJob::RenderJob(View* robloxView,
	...
	, stopped(0)
    , lastVSyncEnabled(false) // Default assumption, will be corrected in first step
    , vsyncInitialized(false)
	, prepareBeginEvent(false)
	, prepareEndEvent(false)
{
    // Initialize TaskScheduler throttling
    RBX::TaskScheduler::singleton().cyclicMinFrameDelta = 1.0 / maxFrameRate;
    RBX::TaskScheduler::singleton().DataModel30fpsThrottle = true;
}
...
{
    if (isAwake) {
        Time::Interval sleep = computeStandardSleepTime(stats, maxFrameRate);
        // FASTLOG3(FLog::Graphics, "RenderJob::sleepTime: maxFPS=%f, isAwake=%d, sleep=%f", (float)maxFrameRate, isAwake, sleep.seconds());
        return sleep;
    }
    else
...
TaskScheduler::StepResult RenderJob::stepDataModelJob(const Stats& stats)
{
    ...
    float currentMaxFPS = CRenderSettingsItem::singleton().getMaxFrameRate();
    setMaxFrameRate(currentMaxFPS);

    // Update TaskScheduler cyclic delta
    if (currentMaxFPS > 0.0f) {
        RBX::TaskScheduler::singleton().cyclicMinFrameDelta = 1.0 / currentMaxFPS;
    } else {
        RBX::TaskScheduler::singleton().cyclicMinFrameDelta = 0.0; // Uncapped
    }
    RBX::TaskScheduler::singleton().DataModel30fpsThrottle = true;

    bool vsync = (CRenderSettingsItem::singleton().getFrameRateCap() == CRenderSettings::FrameRateCap_VSync);
    if (!vsyncInitialized || vsync != lastVSyncEnabled) {
        FASTLOG1(FLog::Graphics, "RenderJob: Updating VSync to %d", vsync);
        lastVSyncEnabled = vsync;
        vsyncInitialized = true;
        marshaller->Submit(boost::bind(&View::SetVSync, robloxView, vsync));
    }
