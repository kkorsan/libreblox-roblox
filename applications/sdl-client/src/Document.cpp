...
void Document::Initialize(SDL_Window* window, bool useChat)
{
...
// implement this check
	...
	{
		std::string err = RBX::format("Exception occurred in Document::executeScript: %s", e.what());
		LogManager::ReportEvent(EVENTLOG_ERROR_TYPE, err.c_str());
		throw;
	}
	catch (std::exception& e)
	...
	{
		std::string err = "Unknown exception occurred in Document::executeScript";
		LogManager::ReportEvent(EVENTLOG_ERROR_TYPE, err.c_str());
		if (GuiService* gs = dataModel->create<GuiService>())
			gs->setUiMessage(GuiService::UIMESSAGE_INFO, "Unable to join game. Please try again later.");
		return;
...
std::string Document::GetSEOStr() const
{
	...
	// Stub for Linux: return default SEO string
	return "A screenshot from ROBLOX. Learn more at https://www.roblox.com";
}
