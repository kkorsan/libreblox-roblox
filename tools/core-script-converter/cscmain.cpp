...
"output   - output path/name (e.g. ../app/engine/src/script/LuaGenCS.inl)\n"
"\n"
"Optional settings:\n"
...
static void rdconfig(const char* cfgfile, char** argv)
{
    ...
    while (true)
    {
        ...
        *q = 0; // kill the '=' sign, because who cares
        ...
        for (v = q + 1; *v && strchr(" \t\r", *v); ++v) {} // find where the value starts

        q = v + strlen(v) - 1; // end of the line
        while ( q > v && strchr(" \n\t\r", *q ) ) { *q-- = 0; } // trim from the end
