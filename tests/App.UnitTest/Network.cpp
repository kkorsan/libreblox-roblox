...
BOOST_AUTO_TEST_CASE(FilterEnabled)
{
	...
	BOOST_TEST_MESSAGE("Waiting for brick to replicate");
	G3D::System::sleep(SLEEPTIME);
	...
	BOOST_TEST_MESSAGE("Giving client-1 a chance to replicate its name change");
	G3D::System::sleep(SLEEPTIME);
...
BOOST_AUTO_TEST_CASE(ShipsInTheNight)
{
	...
	BOOST_TEST_MESSAGE("Waiting for TestPart to replicate");
	G3D::System::sleep(SLEEPTIME);
	...
	BOOST_TEST_MESSAGE("Waiting for client to replicate Blue to server");
	G3D::System::sleep(SLEEPTIME);
	...
	BOOST_TEST_MESSAGE("Waiting for server to replicate Red to client");
	G3D::System::sleep(SLEEPTIME);
	...
	BOOST_TEST_MESSAGE("Waiting for client to replicate Yellow to server");
	G3D::System::sleep(SLEEPTIME);
...
BOOST_AUTO_TEST_CASE(PropertyBounceBack)
{
	...
	BOOST_TEST_MESSAGE("Waiting for client to replicate Red to server");
	G3D::System::sleep(SLEEPTIME);
...
BOOST_AUTO_TEST_CASE(ParentPropChangeArriveInOrder)
{
	...
	BOOST_TEST_MESSAGE("Modifying parent on Server");
	...
	shared_ptr<PartInstance> serverPart3;
	{
        ...
        BOOST_TEST_MESSAGE("Creating serverPart2");
        ...
        BOOST_TEST_MESSAGE("Setting parent to serverPart2");
		childPart->setParent(serverPart2.get());

		serverPart->setParent(NULL);

        BOOST_TEST_MESSAGE("Creating serverPart3");
        ...
        BOOST_TEST_MESSAGE("Setting parent to serverPart3");
...
BOOST_FIXTURE_TEST_CASE(Perf, RBX::Test::PerformanceTestFixture)
{
	...
	BOOST_TEST_MESSAGE("Waiting for LastPart to replicate");
...
{
	BOOST_TEST_MESSAGE("game loaded");
	event->Set();
};
...
BOOST_FIXTURE_TEST_CASE(DeserializeProcessTest, RBX::Test::PerformanceTestFixture)
{
	...
	BOOST_TEST_MESSAGE(RBX::format("time for parts to propagate to server %f seconds", ptime));
...
BOOST_AUTO_TEST_CASE(PropTestServer)
{
    ...
    shared_ptr<PartInstance> serverIns;
    {
        ...
        // Create an instance from the new class on the server
        BOOST_TEST_MESSAGE("Creating instance from TheNewClass...");
...

    {
        ...
        BOOST_TEST_MESSAGE("Changing ExistingProp and TheNewProp...");
...
BOOST_AUTO_TEST_CASE(EventTestServer)
{
...

    {
        ...
        // Fire events
        BOOST_TEST_MESSAGE("Firing ExistingEvent and TheNewEvent...");
