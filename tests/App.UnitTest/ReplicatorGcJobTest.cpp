...
bool SetMemoryLevel(DataModelFixture& dm)
{
	...
	if (!client)
	{
		BOOST_TEST_MESSAGE("Warning: Failed to set memory level, client == null");
		return false;
	}
	...
	if (!replicator)
	{
		BOOST_TEST_MESSAGE("Warning: Failed to set memory level, replicator == null");
		return false;
	}
	...
	if (replicator->getMemoryLevel() != RBX::MemoryStats::MEMORYLEVEL_ALL_CRITICAL_LOW)
	{
		...
		BOOST_TEST_MESSAGE("force setting memory level");
...
BOOST_AUTO_TEST_CASE(MovingPartOutsideOfStreamedAreaCausesItToBeGced)
{
...

    {
        ...
        BOOST_TEST_MESSAGE("move part away");
    }
...

    {
		...
		BOOST_TEST_MESSAGE("move player to trigger GC");
...
BOOST_AUTO_TEST_CASE(MovingPartAndItsParentOutsideOfStreamedAreaShouldNotRemoveThePartTwice)
{
...

    {
        ...
        BOOST_TEST_MESSAGE("move both parent and child parts away");
    }
...
BOOST_AUTO_TEST_CASE(ShipsInTheNightReparentingAndGc)
{
	...
	}
	{
		...
		ScopedAssign<double> assignReplicationLag(NetworkSettings::singleton().incommingReplicationLag, 10000);
		{
            ...
            BOOST_TEST_MESSAGE("reparent MeshO2I to partInside");
			specialMeshOutsideToInside->setParent(partInsideStreamedArea.get());
            BOOST_TEST_MESSAGE("reparent MeshI2O to partToBeOutside");
        ...
        BOOST_TEST_MESSAGE("wait for gc");
		G3D::System::sleep(SLEEPTIME);
    ...
    BOOST_TEST_MESSAGE("wait for replications");
	G3D::System::sleep(SLEEPTIME);
