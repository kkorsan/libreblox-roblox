...
class TestServiceTester : public Tester
{
	...
	void onMessaged(std::string text, shared_ptr<Instance> source, int line)
	{
		...
		std::stringstream ss;
		ss << "[" << (source ? source->getFullName() : "") << ":" << line << "] " << text;
	...
	void onWarning(bool condition, std::string description, shared_ptr<Instance> source, int line)
	{
		...
		std::stringstream ss;
		ss << "[" << (source ? source->getFullName() : "") << ":" << line << "] " << description;
		BOOST_WARN_MESSAGE(condition, ss.str());
	}
	...
	void onChecking(bool condition, std::string description, shared_ptr<Instance> source, int line)
	{
		std::stringstream ss;
		ss << "[" << (source ? source->getFullName() : "") << ":" << line << "] " << description;
		BOOST_CHECK_MESSAGE(condition, ss.str());
	}
...
public:
	...
	{
		BOOST_TEST_MESSAGE(boost::format("Waiting up to %d seconds for the test to end") % timeout);
	}
...
public:
	...
	void run()
	{
		...
		BOOST_REQUIRE_NO_THROW(scriptContext->executeInNewThread(RBX::Security::CmdLine_, ProtectedString::fromTrustedSource(clientScript), "End callback", args));
	...
	virtual void wait()
	{
		BOOST_TEST_MESSAGE(boost::format("Waiting up to %d seconds for the test to end") % timeout.seconds());
...
static bool gatherTestCases(path dir_path, boost::unit_test::test_suite* suite)
{
	...
	for ( directory_iterator iter( dir_path ); iter != end_itr; ++iter )
	{
		...
		if (p.extension() == ".rbxl" || p.extension() == ".rbxlx" || (clientTest && p.extension() == ".clienttest"))
		{
			...
			{
				clientTestFile = p;
				suite->add(boost::unit_test::make_test_case( boost::bind(&FileTester::test, tester), filename_without_spaces.string() + ".clienttest", __FILE__, __LINE__ ));
			}
			else if (p.extension() == ".rbxl")
				suite->add(boost::unit_test::make_test_case( boost::bind(&FileTester::test, tester), filename_without_spaces.string() + ".rbxl", __FILE__, __LINE__ ));
			else
				suite->add(boost::unit_test::make_test_case( boost::bind(&FileTester::test, tester), filename_without_spaces.string() + ".rbxlx", __FILE__, __LINE__ ));
	...
	for ( directory_iterator iter( dir_path ); iter != end_itr; ++iter )
	{
		if (!is_directory(iter->status()))
			continue;
		path p = iter->path();
		boost::unit_test::test_suite* subSuite = new boost::unit_test::test_suite(p.filename().string().c_str(), __FILE__, __LINE__);
		suite->add(subSuite);
